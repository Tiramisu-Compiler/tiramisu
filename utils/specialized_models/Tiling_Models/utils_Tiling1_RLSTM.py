    ...
    def __init__(self, dataset_SC_filename, max_batch_size, filter_func_SC=None, transform_func=None):
        ...
        for function_name in tqdm(self.programs_dict_SC):
            ...
            for schedule_index in range(len(self.programs_dict_SC[function_name]['schedules_list'])):
                ...
                scheduleP_str = re.sub("T\d\([L\d,]+\)", '', sched_str)
            ...
            for elem in schedules_dict.items():
                sched_json = self.programs_dict_SC[function_name]['schedules_list'][elem[0]]
                try:
                    tree, comps_tensor, loops_tensor = get_representation(program_json, sched_json) # Get the program + schedule representation
                except LargeAccessMatices:
                    self.nb_long_access +=1
                    continue   
                
                # for each datapoint append its best tiling loops
                ...
                self.batches_dict[tree_footprint]['output'].append(torch.tensor(encode_tiling(best_schedule_str, len(program_json['iterators']) )))
        ...
        for tree_footprint in self.batches_dict:
            for chunk in range(0,len(self.batches_dict[tree_footprint]['program_names_list']),max_batch_size):  
                ...
                dim_temp_tens = int(temp_tens.shape[0] / 28)
                self.Y.append(torch.reshape(temp_tens, (dim_temp_tens,28)).to(storing_device)) # Append the vector of best tiled loops
...
class Model_Recursive_LSTM_v2(nn.Module):
    def __init__(self, input_size, comp_embed_layer_sizes=[600, 350, 200, 180], drops=[0.225, 0.225, 0.225, 0.225], output_size=1):
        ...
        self.classification_dropouts= nn.ModuleList()
        ...
        for i in range(len(regression_layer_sizes)-1):
            self.classification_layers.append(nn.Linear(regression_layer_sizes[i], regression_layer_sizes[i+1], bias=True))
            nn.init.xavier_uniform_(self.classification_layers[i].weight)
            self.classification_dropouts.append(nn.Dropout(drops[i]))
    ...
    def forward(self, tree_tensors):
        ...
        for i in range(len(self.classification_layers)):
            x = self.classification_layers[i](x)
            x = self.classification_dropouts[i](self.ELU(x))
        out = self.predict(x)
...
def decode_tiling(pos, depth):
    if pos == 0 : # no tiling
        return 0,0
    else :
        if(pos<depth) :
            return pos-1, pos  # pos = b ; a = b-1
        else :
            c = pos-depth+2
            return c-2, c-1, c

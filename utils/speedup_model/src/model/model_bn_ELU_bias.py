...
class Model_BN_ELU_bias(nn.Module):
    def __init__(self, input_size, output_size, hidden_sizes=[10, 10, 10], drops=[0.4, 0.4, 0.4]):
        ...
        for i in range(len(hidden_sizes)-1):
            self.hidden_layers.append(nn.Linear(hidden_sizes[i], hidden_sizes[i+1], bias=True))
    ...
    def forward(self, x):
        ...
        for i in range(len(self.hidden_layers)):
            x = self.dropouts[i](self.ELU(self.batch_norm_layers[i](self.hidden_layers[i](x))))

...
if [ "${USE_LIBJPEG}" = "0" ]; then
    echo_and_run_cmd "make TEST_CXX_FLAGS=\"_GLIBCXX_USE_CXX11_ABI=0\" CXXFLAGS=\"-DHALIDE_NO_JPEG _GLIBCXX_USE_CXX11_ABI=0\" -j $CORES"
else
    echo_and_run_cmd "make clean"
    echo_and_run_cmd "make TEST_CXX_FLAGS=\"_GLIBCXX_USE_CXX11_ABI=0\" CXXFLAGS=\"_GLIBCXX_USE_CXX11_ABI=0\" -j $CORES"
fi

...
def compile_tiramisu_code(prog):
    ...
    subprocess.run(["g++", "-std=c++11", "-fno-rtti", "-DHALIDE_NO_JPEG", "-I%s/include" % TIRAMISU_ROOT, "-I%s/3rdParty/isl/include/" % TIRAMISU_ROOT, "-I%s/3rdParty/Halide/include" % TIRAMISU_ROOT, "-I%s/benchmarks" % TIRAMISU_ROOT, "-I%s/build" % TIRAMISU_ROOT, "-L%s/build" % TIRAMISU_ROOT, "-L%s/3rdParty/isl/build/lib" % TIRAMISU_ROOT, "-L%s/3rdParty/Halide/lib/" % TIRAMISU_ROOT, "-o", str(dst_folder_path / (sched_id + "_generator")), "-ltiramisu", "-lisl", "-lHalide", "-ldl", "-lpthread", "-lz", "-lm", "-Wl,-rpath,${TIRAMISU_ROOT}/build", str(src_folder_path / (sched_id + ".cpp")), "-ltiramisu", "-lisl", "-lHalide", "-ldl", "-lpthread", "-lz", "-lm"], check=True)

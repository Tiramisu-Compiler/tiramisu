...
#         iterators representation + transformations placeholders
    ...
    for iter_i,iterator_name in enumerate(comp_dict['iterators']):
        ...
        iterators_repr.extend([l_code+'Interchanged', 
                               ...
                               l_code+'Reversed', l_code+'Reversed',
                               0, 
                               0,
                               ...
                               l_code+"_1"+'Reversed', l_code+"_1"+'Reversed']) #unrolling is skipped since it is added only once
...
def get_iterators_dict(prog_json):
    iterators_dict={}
    #Get the computation iterators list
    iterators_list=prog_json["iterators"].keys()

    for iterator in iterators_list:
        iterator_dict=prog_json["iterators"]

        iterators_dict[iterator]={'lower_bound':iterator_dict[iterator]['lower_bound'],
                                'upper_bound':iterator_dict[iterator]['upper_bound'],
                                'parent_iterator':iterator_dict[iterator]['parent_iterator'],
                                'child_iterators':iterator_dict[iterator]['child_iterators']}
...
def update_iterators(id, it_list, action_params):
    ...
    if id == 1:
        ...
        for i in keys:
            print("len of action_params is", len(action_params))
            if len(action_params)==7:
                if i>depth_2:
                    if action_params["tiling_loop_1"] and action_params["tiling_loop_2"]:
                            it_list[i+2]=it_list.pop(i)
                    elif action_params["tiling_loop_1"] or action_params["tiling_loop_2"]:   
                            it_list[i+1]=it_list.pop(i)
                   
                
            else:
                if len(action_params)==10:
                        ...
                        if i>depth_3:
                            if action_params["tiling_loop_1"] and action_params["tiling_loop_2"] and action_params["tiling_loop_3"]:   
                                ...
                                if booleans.count(True)==2:
                                    ...
                                    it_list[i+1]=it_list.pop(i)
            print("it_dict after popping only", it_list)            
...
def get_cpp_file(Dataset_path,func_name):
    ...
    original_path = Dataset_path+'/'+func_name 

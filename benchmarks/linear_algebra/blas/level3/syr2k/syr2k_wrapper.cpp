		  ...
		  double * result)
{
	...
	for(int i = 0; i<NN; i++)
	{
		for(int j = 0; j<=i; j++)
		{
			tmp=0;
			for(int k = 0; k<KK; k++)
				tmp += A[i * KK + k] * B[j * KK + k] + B[i * KK + k] * A[j * KK + k];
...
int main(int argc, char** argv)
{
	...
	Halide::Buffer<double> b_B(N_DIM, K_DIM);
	init_buffer(b_B, (double) 1);
	...
	/**
	    ...
	    - B a N_DIM by K_DIM matrix of ones
	...
	*/
	...
	//REFERENCE
	{
		for (int i = 0; i < NB_TESTS; i++)
		{
			...
			if (run_ref)
				syr2k_ref(SIZES(0), SIZES(1), b_A.data(), b_B.data(), b_C.data(), b_alpha.data(), b_beta.data(), b_result_ref.data());
	...
	//TIRAMISU
	{
		for (int i = 0; i < NB_TESTS; ++i)
		{
			...
			if (run_tiramisu)
				syr2k(SIZES.raw_buffer(), b_A.raw_buffer(), b_B.raw_buffer(), b_C.raw_buffer(), b_alpha.raw_buffer(), b_beta.raw_buffer(), b_result.raw_buffer());
	...
	if (run_ref && run_tiramisu)
		compare_buffers("syr2k", b_result, b_result_ref);

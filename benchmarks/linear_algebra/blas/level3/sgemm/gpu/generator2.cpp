...
int main(int argc, char **argv)
{
    ...
    // Two step copy from global to shared (as in Diesel paper)
    ...
    buffer b_A_tmp("b_A_tmp", {R_BLOCK_I}, DATA_PTYPE, a_temporary);
    buffer b_B_tmp("b_B_tmp", {R_BLOCK_J}, DATA_PTYPE, a_temporary);
    ...
    b_A_tmp.tag_gpu_local();
    b_B_tmp.tag_gpu_local();
    ...
    computation c_A_glb_to_tmp({i0, j0, k0_skiplast, i1}, c_A_glb(i0, j0, k0_skiplast + 1, i1));
    computation c_A_tmp_to_shr({i0, j0, k0_skiplast, i1}, c_A_glb_to_tmp(i0, j0, k0_skiplast, i1));
    ...
    computation c_B_glb_to_tmp({i0, j0, k0_skiplast, j1}, c_B_glb(i0, j0, k0_skiplast + 1, j1));
    computation c_B_tmp_to_shr({i0, j0, k0_skiplast, j1}, c_B_glb_to_tmp(i0, j0, k0_skiplast, j1));
    ...
    computation c_A_tmp_dec({i0, j0}, allocate(b_A_tmp));
    computation c_A_reg_dec({i0, j0}, allocate(b_A_reg));
    computation c_B_shr_dec({i0, j0}, allocate(b_B_shr));
    computation c_B_tmp_dec({i0, j0}, allocate(b_B_tmp));
    ...
    block kernel_block({&c_acc_dec, &c_acc_init, &c_acc, &c_C,
            &c_A_shr_dec, &c_A_tmp_dec, &c_A_reg_dec,
            &c_B_shr_dec, &c_B_tmp_dec, &c_B_reg_dec,
            &c_A_glb_to_shr_pre, &c_A_glb_to_tmp, &c_A_tmp_to_shr, &c_A_shr_to_reg,
            &c_B_glb_to_shr_pre, &c_B_glb_to_tmp, &c_B_tmp_to_shr, &c_B_shr_to_reg,
            &c_sync1, &c_sync2, &c_sync3});
    ...
    copy_A_to_device.then(copy_B_to_device, computation::root)
                    ...
                    .then(c_A_tmp_dec, j01)
                    .then(c_B_tmp_dec, j01)
                    ...
                    .then(c_sync1, j01)
                    .then(c_A_glb_to_tmp, j01)
                    .then(c_B_glb_to_tmp, k0)
                    .then(c_B_shr_to_reg, k0)
                    .then(c_A_shr_to_reg, k1)
                    .then(c_acc, i1)
                    .then(c_sync2, k0)
                    .then(c_A_tmp_to_shr, k0)
                    .then(c_B_tmp_to_shr, k0)
                    .then(c_sync3, k0)
                    .then(c_C, j01)
    ...
    // Note the transpose:
    c_A_glb_to_shr_pre.store_in(&b_A_shr, {j0 % BLOCK, i0 % BLOCK * R_BLOCK_I + i1});
    c_A_glb_to_tmp.store_in(&b_A_tmp, {i1});
    c_A_tmp_to_shr.store_in(&b_A_shr, {j0 % BLOCK, i0 % BLOCK * R_BLOCK_I + i1});
    ...
    c_B_glb_to_shr_pre.store_in(&b_B_shr, {i0 % BLOCK, j1 * BLOCK + j0 % BLOCK});
    c_B_glb_to_tmp.store_in(&b_B_tmp, {j1});
    c_B_tmp_to_shr.store_in(&b_B_shr, {i0 % BLOCK, j1 * BLOCK + j0 % BLOCK});

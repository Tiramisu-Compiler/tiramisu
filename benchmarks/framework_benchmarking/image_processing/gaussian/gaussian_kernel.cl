...
"__kernel void kernel0(__global unsigned char *conv, __global float *kernelX, __global float *kernelY, __global unsigned char *src, __global unsigned char *temp, int step, int rows, int kernelY_length, int kernelX_length, int cols)\n"
"{\n"
...
"            temp[((t0 + c0) * step + (c2 + c5)) * 3 + t1] = private_prod1;\n"
"            for (int c6 = 0; c6 < kernelY_length; c6 += 1)\n"
"              private_prod2 += (temp[((t0 + c0) * step + (c2 + c5)) * 3 + t1] * kernelY[c6]);\n"
"            shared_conv[t0][c5][t1] = private_prod2;\n"
"          }\n"
"        barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);\n"

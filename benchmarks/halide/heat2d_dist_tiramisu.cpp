...
int main(int argc, char **argv)
{
    ...
    computation init("[NB, MB]->{init[i,j]: 1<=i<NB and 1<=j<MB}", expr(0.0f), true, data_type, &heat2d_tiramisu);
    ...
    if (NUM_CPU_RANKS+NUM_GPU_RANKS == 1) {
      ...
      var i3("i3"), i4("i4");
      init.split(i2, (N-2)/(NUM_CPU_RANKS+NUM_GPU_RANKS)/8, i3, i4);
      out_comp.split(i2, (N-2)/(NUM_CPU_RANKS+NUM_GPU_RANKS)/8, i3, i4);
      init.tag_parallel_level(i3);
    }
    ...
    if (NUM_CPU_RANKS+NUM_GPU_RANKS > 1) {
      send_recv_bkwd = computation::create_xfer("[N,M,CRANKS]->{send_bkwd[q,i,j]: 1<=q<CRANKS and 0<=i<1 and 0<=j<=M}",
						"[N,M,CRANKS]->{recv_bkwd[q,i,j]: 0<=q<CRANKS-1 and 0<=i<1 and 0<=j<M}",
						q-1, q+1, xfer_prop(data_type, {ASYNC, BLOCK, MPI}),
						xfer_prop(data_type, {SYNC, BLOCK, MPI}), in(i,j), &heat2d_tiramisu);
      
      send_recv_fwd = computation::create_xfer("[N,M,CRANKS]->{send_fwd[q,i,j]: 0<=q<CRANKS-1 and 0<=i<1 and 0<=j<M}",
					       "[N,M,CRANKS]->{recv_fwd[q,i,j]: 1<=q<CRANKS and 0<=i<1 and 0<=j<M}",
						  q+1, q-1, xfer_prop(data_type, {ASYNC, BLOCK, MPI}),
    xfer_prop(data_type, {SYNC, BLOCK, MPI}), in(i + N / (NUM_CPU_RANKS + NUM_GPU_RANKS) - 1, j), &heat2d_tiramisu);
    }
    // Optimizations
    ...
    if (NUM_CPU_RANKS+NUM_GPU_RANKS > 1) {
      send_recv_bkwd.s->collapse_many({collapse_group(2, (cit_type)0, (cit_type)(M-2)), collapse_group(1, (cit_type)0, (cit_type)1)});
      send_recv_bkwd.r->collapse_many({collapse_group(2, (cit_type)0, (cit_type)(M-2)), collapse_group(1, (cit_type)0, (cit_type)1)});
      send_recv_fwd.s->collapse_many({collapse_group(2, (cit_type)0, (cit_type)(M-2)), collapse_group(1, (cit_type)0, (cit_type)1)});
      send_recv_fwd.r->collapse_many({collapse_group(2, (cit_type)0, (cit_type)(M-2)), collapse_group(1, (cit_type)0, (cit_type)1)});
    ...
    // Buffers
    buffer buff_in{"buff_in", {N/(NUM_CPU_RANKS+NUM_GPU_RANKS) + 2, M}, data_type, a_input, &heat2d_tiramisu};
    buffer buff_out{"buff_out", {N/(NUM_CPU_RANKS+NUM_GPU_RANKS), M}, data_type, a_output, &heat2d_tiramisu};

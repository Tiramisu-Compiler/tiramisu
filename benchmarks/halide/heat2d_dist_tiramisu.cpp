...
int main(int argc, char **argv)
{
    ...
    xfer send_recv_bkwd = computation::create_xfer("[N,CRANKS]->{send_bkwd[q,i,j]: 1<=q<CRANKS and 0<=i<1 and 0<=j<N}",
    ...
    xfer send_recv_fwd = computation::create_xfer("[N,CRANKS]->{send_fwd[q,i,j]: 0<=q<CRANKS-1 and 0<=i<1 and 0<=j<N}",

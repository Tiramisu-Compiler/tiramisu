...
//            .parallel(x_outer);
    ...
    gaussian_x.gpu_tile(x, y, x_inner, y_inner, x_outer, y_outer, 16, 16);
    gaussian.gpu_tile(x, y, x_inner, y_inner, x_outer, y_outer, 16, 16);

    Halide::Target target = Halide::get_host_target();
    target.set_feature(Target::Feature::CUDA, true);

    gaussian.compile_to_object("build/generated_fct_gaussian_ref.o", {in, kernelX, kernelY}, "gaussian_ref", target);

    gaussian.compile_to_lowered_stmt("build/generated_fct_gaussian_ref.txt", {in, kernelX, kernelY}, Text, target);

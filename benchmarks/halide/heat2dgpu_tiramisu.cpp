...
int main(int argc, char **argv)
{
    ...
    constant MB{"MB", M - 1, it_type, true, nullptr, 0, &heat2d_tiramisu};

    var i("i"), j("j"), i0("i0"), j0("j0"), i1("i1"), j1("j1");

    computation in{"[N, M] -> {in[i, j]: 0 <= i < N and 0 <= j < M}", expr(), false, data_type, &heat2d_tiramisu};
    computation out_init{"[N, M] -> {out_init[i, j]: 0 <= i < N and 0 <= j < M}", 0.f, true, data_type, &heat2d_tiramisu};
    expr comp_expr = in(i, j) * alpha + (in(i + 1, j) + in(i, j + 1) + in(i - 1, j) + in(i, j - 1)) * beta;
    computation out_comp{"[NB, MB] -> {out_comp[i, j]: 1 <= i < NB and 1 <= j < MB}", comp_expr, true, data_type, &heat2d_tiramisu};

    out_comp.after(out_init, computation::root);
    ...
    out_init.set_access("{out_init[i,j] -> buff_out[i,j]}");
    out_comp.set_access("{out_comp[i,j] -> buff_out[i,j]}");

    out_comp.tile(i, j, block_size, block_size, i0, j0, i1, j1);

    computation in_gpu{"[N, M] -> {in_gpu[i, j]: 0 <= i < N and 0 <= j < M}", expr(), false, data_type, &heat2d_tiramisu};
    ...
    computation shared_dec{"[NB, MB] -> {shared_init_center[i0, j0, i1, j1] : " + whole_condition + "}",
                           expr(o_allocate, buff_shared.get_name()), true, p_none, &heat2d_tiramisu};

    computation shared_init_top_edge{"[NB, MB] -> {shared_init_top_edge[i, j] : 1 <= i < NB and 1 <= j < MB and (i = 1 or (i >= " + std::to_string(block_size) + " and i % " + std::to_string(block_size) + " = 0))}",
                                     in_gpu(i - 1, j),
                                     true, data_type, &heat2d_tiramisu};
    shared_init_top_edge.tile(i, j, block_size, block_size, i0, j0, i1, j1);

    computation shared_init_left_edge{"[NB, MB] -> {shared_init_left_edge[i, j] : 1 <= i < NB and 1 <= j < MB and (j = 1 or (i >= " + std::to_string(block_size) + " and j % " + std::to_string(block_size) + " = 0))}",
                                     in_gpu(i, j - 1),
                                     true, data_type, &heat2d_tiramisu};
    shared_init_left_edge.tile(i, j, block_size, block_size, i0, j0, i1, j1);

    computation shared_init_bot_edge{"[NB, MB] -> {shared_init_bot_edge[i, j] : 1 <= i < NB and 1 <= j < MB and ((i + 1) % " + std::to_string(block_size) + " = 0 or i + 1 = NB)}",
                                     in_gpu(i + 1, j),
                                     true, data_type, &heat2d_tiramisu};
    shared_init_bot_edge.tile(i, j, block_size, block_size, i0, j0, i1, j1);

    computation shared_init_right_edge{"[NB, MB] -> {shared_init_right_edge[i, j] : 1 <= i < NB and 1 <= j < MB and ((j + 1) % " + std::to_string(block_size) + " = 0 or j + 1 = MB)}",
                                     in_gpu(i, j + 1),
                                     true, data_type, &heat2d_tiramisu};
    shared_init_right_edge.tile(i, j, block_size, block_size, i0, j0, i1, j1);

    computation shared_init_center{"[NB, MB] -> {shared_init_center[i, j] : 1 <= i < NB and 1 <= j < MB}",
                                   in_gpu(i, j),
                                   true, data_type, &heat2d_tiramisu};
    ...
    computation shared_buff{"{shared_buff[i, j]: 0 <= i < " + std::to_string(block_size) + " and 0 <= j < " + std::to_string(block_size) + "}",
                            expr(), false, data_type, &heat2d_tiramisu};
    shared_buff.set_access("{shared_buff[i, j] -> buff_shared[i + 1, j + 1]}");
    out_comp.set_expression(out_comp.get_expr().substitute_access(in.get_name(), shared_buff.get_name()));

    computation synchronize{"[MB, NB] -> {synchronize[i, j]: 1 <= i < NB and 1 <= j < MB}", tiramisu::sync{}, true, p_none, &heat2d_tiramisu};
    synchronize.tile(i, j, block_size, block_size, i0, j0, i1, j1);


    shared_dec.between(out_init, computation::root, out_comp, j1);
    ...
    in_gpu.set_access("{in_gpu[i,j] -> buff_in_gpu[i,j]}");
    out_init.set_access("{out_init[i,j] -> buff_out_gpu[i,j]}");
    out_comp.set_access("{out_comp[i,j] -> buff_out_gpu[i,j]}");
    out_comp.set_expression(out_comp.get_expr().substitute_access(in.get_name(), in_gpu.get_name()));

    out_init.gpu_tile(i, j, block_size, block_size);

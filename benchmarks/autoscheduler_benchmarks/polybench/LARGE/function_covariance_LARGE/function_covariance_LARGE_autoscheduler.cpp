...
{
    tiramisu::init("function_covariance_LARGE");
    ...
    //Iteration variables    
    var i("i", 0, 1200), j("j", 0, 1200), k("k", 0, 1400), l("l", 0, 1400);
    ...
    computation cov_init("{cov_init[i,j]: 0<=i<1200 and i<=j<1200}", expr(0.0), true, p_float64, global::get_implicit_function());
    
    computation cov_prod("{cov_prod[i,j,k]: 0<=i<1200 and i<=j<1200 and 0<=k<1400}", expr(), true, p_float64, global::get_implicit_function());
    cov_prod.set_expression(cov(i,j) + data(k,i)*data(k,j));

    computation cov_div("{cov_div[i,j]: 0<=i<1200 and i<=j<1200}", expr(0.0), true, p_float64, global::get_implicit_function());
    cov_div.set_expression(cov(i,j)/expr(cast(p_float64, 1400-1)));

    computation cov_sym("{cov_sym[i,j]: 0<=i<1200 and i<=j<1200}", expr(0.0), true, p_float64, global::get_implicit_function());
    ...
    buffer b_cov("b_cov", {1200,1200}, p_float64, a_output);   
	...
	auto_scheduler::evaluate_by_execution *exec_eval = new auto_scheduler::evaluate_by_execution({&b_data, &b_cov}, "function_covariance_LARGE.o", "./function_covariance_LARGE_wrapper");
	...
	if (explore_by_execution){
		...
		as.sample_search_space("./function_covariance_LARGE_explored_schedules.json", true);
	}else{

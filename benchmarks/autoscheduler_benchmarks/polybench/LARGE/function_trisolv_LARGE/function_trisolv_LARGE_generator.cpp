...
int main(int argc, char **argv)
{
    ...
    computation x_init("{x_init[i]: 0<=i<2000 }", expr(), true, p_float64, global::get_implicit_function());
    x_init.set_expression(b(i));
    computation x_sub("{x_sub[i,j]: 0<=i<2000 and 0<=j<i}", expr(), true, p_float64, global::get_implicit_function());
    x_sub.set_expression(x(i) - L(i,j) * x(j));
    computation x_out("{x_out[i]: 0<=i<2000 }", expr(), true, p_float64, global::get_implicit_function());

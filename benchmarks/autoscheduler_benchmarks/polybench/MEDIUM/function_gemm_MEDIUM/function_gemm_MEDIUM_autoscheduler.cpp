...
{
    tiramisu::init("function_gemm_MEDIUM");

    //Iteration variables    
    var i("i", 0, 200), j("j", 0, 220), k("k", 0, 240);
    ...
    // -------------------------------------------------------
    //Input Buffers
    buffer b_A("b_A", {200,240}, p_float64, a_input);
    buffer b_B("b_B", {240,220}, p_float64, a_input);
    buffer b_C("b_C", {200,220}, p_float64, a_output);
	...
	auto_scheduler::evaluate_by_execution *exec_eval = new auto_scheduler::evaluate_by_execution({&b_A, &b_B, &b_C}, "function_gemm_MEDIUM.o", "./function_gemm_MEDIUM_wrapper");
	...
	if (explore_by_execution){
		...
		as.sample_search_space("./function_gemm_MEDIUM_explored_schedules.json", true);
	}else{

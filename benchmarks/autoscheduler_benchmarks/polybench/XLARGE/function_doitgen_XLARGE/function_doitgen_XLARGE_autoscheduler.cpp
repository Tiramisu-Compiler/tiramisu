...
int main(int argc, char **argv)
 {
  ...
  // Iteration variables
  var r("r", 0, 250), q("q", 0, 220), p("p", 0, 270), s("s", 0, 270);
  ...
  // -------------------------------------------------------
  // Input Buffers
  buffer b_A("b_A", {250, 220, 270}, p_float64, a_output);
  buffer b_sum("b_sum", {270}, p_float64, a_temporary);
  buffer b_x("b_x", {270, 270}, p_float64, a_input);
  ...
  A_out.store_in(&b_A, {r, q, p});
	...
	auto_scheduler::evaluate_by_execution *exec_eval = new auto_scheduler::evaluate_by_execution({&b_A, &b_x}, "function_doitgen_XLARGE.o", "./function_doitgen_XLARGE_wrapper");
	...
	if (explore_by_execution){
		...
		as.sample_search_space("./function_doitgen_XLARGE_explored_schedules.json", true);
	}else{

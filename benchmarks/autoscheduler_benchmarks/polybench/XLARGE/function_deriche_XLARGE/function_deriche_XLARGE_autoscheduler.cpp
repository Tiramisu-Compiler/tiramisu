...
int main(int argc, char **argv)
 {
    ...
    tiramisu::init("function_deriche_XLARGE");


    // -------------------------------------------------------
    // Layer I
    // ------------------------------------------------------- 
    constant HH("HH", 4320), WW("WW",7680);


    //Iteration variables    
    var i("i", 0, 7680), j("j", 0, 4320);
	...
	auto_scheduler::evaluate_by_execution *exec_eval = new auto_scheduler::evaluate_by_execution({&b_imgIn, &b_imgOut}, "function_deriche_XLARGE.o", "./function_deriche_XLARGE_wrapper");
	...
	if (explore_by_execution){
		...
		as.sample_search_space("./function_deriche_XLARGE_explored_schedules.json", true);
	}else{

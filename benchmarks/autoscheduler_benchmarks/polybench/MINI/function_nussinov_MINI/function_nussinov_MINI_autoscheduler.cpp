...

{
    ...
    //Computations
    computation table_1("{table_1[i,j]: -60+1<=i<1 and 1-i<=j<60 and 0<=j-1}", expr(), true, p_float64, global::get_implicit_function());
    table_1.set_expression(expr(o_max, table(-i, j), table(-i, j-1)));
    computation table_2("{table_2[i,j]: -60+1<=i<1 and 1-i<=j<60 and 1-i<60}", expr(), true, p_float64, global::get_implicit_function());
    table_2.set_expression(expr(o_max, table(-i, j), table(1-i, j)));
    computation table_3("{table_3[i,j]: -60+1<=i<1 and 1-i<=j<60 and 0<=j-1 and 1-i<60 and -i<j-1}", expr(), true, p_float64, global::get_implicit_function());
    table_3.set_expression(expr(o_max, table(-i, j), table(1-i, j-1)+((seq(-i)+seq(j))==3.0)));
    computation table_4("{table_4[i,j]: -60+1<=i<1 and 1-i<=j<60 and 0<=j-1 and 1-i<60 and -i>=j-1}", expr(), true, p_float64, global::get_implicit_function());
    table_4.set_expression(expr(o_max, table(-i, j), table(1-i, j-1)));
    computation table_5("{table_5[i,j,k]: -60+1<=i<1 and 1-i<=j<60 and 1-i<=k<j}", expr(), true, p_float64, global::get_implicit_function());
	...
	auto_scheduler::evaluate_by_execution *exec_eval = new auto_scheduler::evaluate_by_execution({&b_table, &b_seq}, "function_nussinov_MINI.o", "./function_nussinov_MINI_wrapper");
	...
	if (explore_by_execution){
		...
		as.sample_search_space("./function_nussinov_MINI_explored_schedules.json", true);
	}else{

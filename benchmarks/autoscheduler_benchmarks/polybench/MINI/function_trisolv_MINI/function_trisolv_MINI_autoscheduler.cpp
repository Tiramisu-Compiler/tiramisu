...
{
    tiramisu::init("function_trisolv_MINI");
    ...
    computation x_init("{x_init[i]: 0<=i<40 }", expr(), true, p_float64, global::get_implicit_function());
    x_init.set_expression(b(i));
    computation x_sub("{x_sub[i,j]: 0<=i<40 and 0<=j<i}", expr(), true, p_float64, global::get_implicit_function());
    x_sub.set_expression(x(i) - L(i,j) * x(j));
    computation x_out("{x_out[i]: 0<=i<40 }", expr(), true, p_float64, global::get_implicit_function());
    ...
    // -------------------------------------------------------
    //Input Buffers
    buffer b_L("b_L", {40,40}, p_float64, a_input);    
	...
	auto_scheduler::evaluate_by_execution *exec_eval = new auto_scheduler::evaluate_by_execution({&b_L, &b_b, &b_x}, "function_trisolv_MINI.o", "./function_trisolv_MINI_wrapper");
	...
	if (explore_by_execution){
		...
		as.sample_search_space("./function_trisolv_MINI_explored_schedules.json", true);
	}else{

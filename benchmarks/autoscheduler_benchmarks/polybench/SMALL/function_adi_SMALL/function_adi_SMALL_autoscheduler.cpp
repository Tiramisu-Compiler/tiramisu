...

{
    ...
    tiramisu::init("function_adi_SMALL");
    // -------------------------------------------------------
    // Layer I
    // ------------------------------------------------------- 

    //Iteration variables    
    var i("i", 1, 60-1), j("j", 1, 60-1), t("t", 1, 40+1), j_reversed("j_reversed", -60+2, 0);
    var i_f("i_f", 0, 60), j_f("j_f", 0, 60);
    ...
    // -------------------------------------------------------
    //Input Buffers
    buffer b_u("b_u", {60,60}, p_float64, a_output);    
    buffer b_p("b_p", {60,60}, p_float64, a_temporary);    
    buffer b_q("b_q", {60,60}, p_float64, a_temporary);    
    buffer b_v("b_v", {60,60}, p_float64, a_temporary);    
    ...
    v_col_last.store_in(&b_v, {60-1,i});
    ...
    u_row_last.store_in(&b_u,{i,60-1});
	...
	auto_scheduler::evaluate_by_execution *exec_eval = new auto_scheduler::evaluate_by_execution({&b_u}, "function_adi_SMALL.o", "./function_adi_SMALL_wrapper");
	...
	if (explore_by_execution){
		...
		as.sample_search_space("./function_adi_SMALL_explored_schedules.json", true);
	}else{

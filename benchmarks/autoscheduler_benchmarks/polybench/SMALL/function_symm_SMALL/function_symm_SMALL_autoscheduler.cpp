...
{
    tiramisu::init("function_symm_SMALL");
    ...
    //Computations
    computation temp_init("{temp_init[i,j]: 0<=i<60 and 0<=j<80}", expr(), true, p_float64, global::get_implicit_function());
    temp_init.set_expression(0.0);
    computation temp_comp("{temp_comp[i,j,k]: 0<=i<60 and 0<=j<80 and 0<=k<i}", expr(), true, p_float64, global::get_implicit_function());
    temp_comp.set_expression(temp(0)+A(i,k)*B(k,j));
    computation C_r("{C_r[i,j,k]: 0<=i<60 and 0<=j<80 and 0<=k<i}", expr(), true, p_float64, global::get_implicit_function());
    C_r.set_expression(C(k,j) + B(i,j)*A(i,k)*1.5);
    computation C_out("{C_out[i,j]: 0<=i<60 and 0<=j<80}", expr(), true, p_float64, global::get_implicit_function());
    ...
    buffer b_B("b_B", {60,80}, p_float64, a_input);
    buffer b_C("b_C", {60,80}, p_float64, a_output);
	...
	auto_scheduler::evaluate_by_execution *exec_eval = new auto_scheduler::evaluate_by_execution({&b_A, &b_B, &b_C}, "function_symm_SMALL.o", "./function_symm_SMALL_wrapper");
	...
	if (explore_by_execution){
		...
		as.sample_search_space("./function_symm_SMALL_explored_schedules.json", true);
	}else{

...

{
    ...
    //Computations
    computation table_1("{table_1[i,j]: -180+1<=i<1 and 1-i<=j<180 and 0<=j-1}", expr(), true, p_int32, global::get_implicit_function());
    table_1.set_expression(expr(o_max, table(-i, j), table(-i, j-1)));
    computation table_2("{table_2[i,j]: -180+1<=i<1 and 1-i<=j<180 and 1-i<180}", expr(), true, p_int32, global::get_implicit_function());
    table_2.set_expression(expr(o_max, table(-i, j), table(1-i, j)));
    computation table_3("{table_3[i,j]: -180+1<=i<1 and 1-i<=j<180 and 0<=j-1 and 1-i<180 and -i<j-1}", expr(), true, p_int32, global::get_implicit_function());
    table_3.set_expression(expr(o_max, table(-i, j), table(1-i, j-1)+cast(p_int32, ((seq(-i)+seq(j))==3))));
    computation table_4("{table_4[i,j]: -180+1<=i<1 and 1-i<=j<180 and 0<=j-1 and 1-i<180 and -i>=j-1}", expr(), true, p_int32, global::get_implicit_function());
    table_4.set_expression(expr(o_max, table(-i, j), table(1-i, j-1)));
    computation table_5("{table_5[i,j,k]: -180+1<=i<1 and 1-i<=j<180 and 1-i<=k<j}", expr(), true, p_int32, global::get_implicit_function());
    ...
    // -------------------------------------------------------
    //Input Buffers
    buffer b_table("b_table", {180,180}, p_int32, a_output);    
    buffer b_seq("b_seq", {180}, p_int32, a_input);    

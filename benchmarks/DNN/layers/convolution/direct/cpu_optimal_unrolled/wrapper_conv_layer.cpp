...
int main(int, char**)
{
	...
	else
	{
		...
		{
			importCSRFromFileAsDense(CSR_FILE_NAME, &weights_buf, &used_FOUT, &used_FIN, &used_K, &n);
		}
		else
		{
			used_FOUT = FOut;
		...
		Halide::Buffer<float> filter2(weights_buf, FOUT_BLOCKING, FIN_BLOCKING, K, K, FIN_NB_BLOCKS, FOUT_NB_BLOCKS);

		for (int fout = 0; fout < FOut; ++fout)
			for (int fin = 0; fin < FIn; ++fin)
				for (int k_y = 0; k_y < K; ++k_y)
					for (int k_x = 0; k_x < K; ++k_x)
						filter(fout%FOUT_BLOCKING, fin%FIN_BLOCKING, k_x, k_y, fin/FIN_BLOCKING, fout/FOUT_BLOCKING) = filter2(fout%FOUT_BLOCKING, fin%FIN_BLOCKING, k_x, k_y, fin/FIN_BLOCKING, fout/FOUT_BLOCKING);
	}

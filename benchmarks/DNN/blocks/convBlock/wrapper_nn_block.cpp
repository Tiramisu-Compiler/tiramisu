...
#include <time.h>
#include "configure.h"
#include "generated_conv_block.o.h"
...
int main(int, char**)
{
    ...
    Halide::Buffer<float> maxpool_tiramisu(N-2*K, N-2*K, FOut, BATCH_SIZE);
...
std::ofstream resultfile;
  resultfile.open ("/home/radja/Desktop/resultTiramisu.txt");

for (int n = 0; n < BATCH_SIZE; ++n)
	for (int z = 0; z < FOut; ++z) 
		for (int y = 0; y < N-2*K; ++y)	       
			for (int x = 0; x < N-2*K; ++x)	resultfile <<maxpool_tiramisu(x, y, z, n);
  ...
  resultfile.close();
    ...
    std::cout << "\t\tTiramisu conv2 duration" << ": " << median(duration_vector_2)/1000 << "; " << std::endl;
    std::cout << "\t\t Result" << ": ";
...
/*for (int n = 0; n < BATCH_SIZE; ++n)
	for (int z = 0; z < FIn; ++z) {
		for (int y = 0; y < N+K; ++y){	       
			for (int x = 0; x < N+K; ++x)
			    printf("%d ,",(int)input(x, y, z, n) );printf("\n"); } printf("\n");}
...
for (int n = 0; n < BATCH_SIZE; ++n)
	for (int z = 0; z < FOut; ++z) {
		for (int y = 0; y < N; ++y){	       
			for (int x = 0; x < N; ++x)
			    printf("%d ,",(int)conv(x, y, z, n) );printf("\n"); } printf("\n");}

//  print_buffer(conv2_tiramisu);
for (int n = 0; n < BATCH_SIZE; ++n)
	for (int z = 0; z < FOut; ++z) {
		for (int y = 0; y < N-K; ++y){	       
			for (int x = 0; x < N-K; ++x)
			    printf("%d ,",(int)conv2_tiramisu(x, y, z, n) );printf("\n"); } printf("\n");}*/


ifstream in("/home/radja/Desktop/resultTiramisu.txt"); 
ifstream in2("/home/radja/Desktop/result.txt");

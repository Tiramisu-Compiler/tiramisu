...
int main(int argc, char **argv)
{
  ...
  x.store_in(&buf_x_cpu);
   ...
   biases.store_in(&buf_biases_cpu);
    ...
    computation c({l, s, k, i}, DATA_TYPE_P);//DATA_TYPE_P
...
buffer buf_c("buf_c", {SEQ_LENGTH + 1, NUM_LAYERS, BATCH_SIZE, FEATURE_SIZE}, p_float32, a_temporary);
  ...
  h.store_in(&buf_h, {s + 1, l + 1, k, i});
c.store_in(&buf_c, {s + 1, l, k, i});
  ...
  computation h_offset({l, s, k, i}, h(l, s, k, i));
  computation weights_T_offset({l, w_t_i, j, i_merged}, weights_T(l, w_t_i, j, i_merged));
  computation tmp_offset({s, k, i_merged}, tmp(s, k, i_merged));

  h_offset.store_in(h.get_buffer(),{s + 1, l + 1, k, i});
  weights_T_offset.store_in(weights_T.get_buffer(),{l, w_t_i, j, i_merged});
  tmp_offset.store_in(tmp.get_buffer() ,{s, k, i_merged});


   computation sum1({l, s0},
        mkl_gemm(*h_offset.get_buffer(), *weights_T_offset.get_buffer(), *tmp_offset.get_buffer(),
  ...
  computation h_offset2({l, s, k, i}, h(l, s, k, i));
  computation weights_T_offset2({l, w_t_i, j, i_merged}, weights_T(l, w_t_i, j, i_merged));
  computation tmp_offset2({s, k, i_merged}, tmp(s, k, i_merged));

  h_offset2.store_in(h.get_buffer(),{s + 1, l + 1, k, i});
  weights_T_offset2.store_in(weights_T.get_buffer(),{l, w_t_i, j, i_merged});
  tmp_offset2.store_in(tmp.get_buffer() ,{s, k, i_merged});
    
    computation sum2({l, s},
       mkl_gemm(*h.get_buffer(), *weights_T.get_buffer(), *tmp.get_buffer(),
   ...
   weights_T.tile(j, i_merged, 16, 16);
    ...
    time_start
            ...
            .then(h_offset, computation::root)         
            .then(weights_T_offset,computation::root )
            .then(tmp_offset, computation::root)           
            .then(sum1, computation::root)

          .then(h_offset2, computation::root)         
            .then(weights_T_offset2,computation::root )
            .then(tmp_offset2, computation::root)     
            .then(sum2, l_s)//l_s
            .then(sig_i,s1 )//s1
            .then(sig_f,i1)//i1
            .then(tnh_z, i1)//i1

...
#include "generated_sparse_conv_relu_conv_relu_256_512_28_tiramisu.o.h"
...
int main(int, char **)
{
  ...
  int FNNZ1 = generateCSRWeights(&filter_values1, WEIGHTS_DENSITY1, &filter_idx1, &filter_finptr1, K, FIn, FOut, FIN1_BLOCKING, 1);
  ...
  Halide::Buffer<float> b_result(N + 2 * PAD_OUTPUT, N + 2 * PAD_OUTPUT, FOut, BATCH_SIZE);
  ...
  for (int i = 0; i < NB_TESTS; i++)
  {
    auto start2 = std::chrono::high_resolution_clock::now();
		...
		sparse_conv_relu_conv_relu_256_512_28_tiramisu(
      b_SIZES.raw_buffer(),
      b_input.raw_buffer(),
  ...
  if (100*(((double)nb_correct)/(BATCH_SIZE * FOut * (N) * (N)))<100)
    std::cout << 0;
  else
    std::cout << 1;

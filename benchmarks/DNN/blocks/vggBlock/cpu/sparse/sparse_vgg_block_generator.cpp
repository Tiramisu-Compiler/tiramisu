...
int main(int argc, char **argv)
{
    ...
    function sparse_vgg_block("sparse_vgg_block");
    ...
    constant maxpool_height("maxpool_height", N/2, p_int32, true, NULL, 0, &sparse_vgg_block);
    constant maxpool_width("maxpool_width", N/2, p_int32, true, NULL, 0, &sparse_vgg_block);
    ...
    computation conv1_init_zero("[BATCHSIZE,F_Out,height,width]->{conv1_init_zero[b,fout,output_y,output_x]: 0<=b<BATCHSIZE and 0<=fout<F_Out and 0<=output_y<height and 0<=output_x<width}",  cast(p_float32, 0), true, p_float32, &sparse_vgg_block);
    computation conv1_add_bias("[BATCHSIZE,F_Out]->{conv1_add_bias[b,fout,y_b,x_b,yy,xx]: 0<=b<BATCHSIZE and 0<=fout<F_Out and 0<=x_b<"+X_NB_BL1s+" and 0<=xx<"+X_BL1s+" and 0<=y_b<"+Y_NB_BL1s+" and 0<=yy<"+Y_BL1s+"}",  expr(), true, p_float32, &sparse_vgg_block);
    conv1_add_bias.set_expression(expr(o_max, 0.f, conv1_add_bias(b, fout, y_b, x_b, yy, xx) + c_bias1(fout)));

    computation convolve1("[BATCHSIZE,F_Out,k1_range0,k1_range1]->{convolve1[b,fout,y_b,x_b,k1,yy,xx]: 0<=b<BATCHSIZE and 0<=fout<F_Out and 0<=x_b<"+X_NB_BL1s+" and 0<=xx<"+X_BL1s+" and 0<=y_b<"+Y_NB_BL1s+" and 0<=yy<"+Y_BL1s+" and k1_range0<=k1<k1_range1}", expr(), true, p_float32, &sparse_vgg_block);

    // Loop invariants
    constant filter_k1("filter_k1", c_filter_idx1(k1), p_int32, false, &convolve1, 4, &sparse_vgg_block);

    constant k1_range0("k1_range0", c_filter_finptr1(fout), p_int32, false, &convolve1, 2, &sparse_vgg_block);
    constant k1_range1("k1_range1", c_filter_finptr1(fout + 1), p_int32, false, &convolve1, 2, &sparse_vgg_block);

    convolve1.set_expression(convolve1(b, fout, y_b, x_b, k1, yy, xx) + c_input(b, (y_b * Y_BL1 + yy) * (N + 2) + x_b * X_BL1 + xx) * c_filter_values1(k1));

    // Second convolution + ReLU
    computation maxpool_init("[BATCHSIZE,F_Out,maxpool_height,maxpool_width]->{maxpool_init[b,fout,output_y,output_x]: 0<=b<BATCHSIZE and 0<=fout<F_Out and 0<=output_y<maxpool_height and 0<=output_x<maxpool_width}",  cast(p_float32, 0), true, p_float32, &sparse_vgg_block);
    ...
    buffer b_workspace("b_workspace", {Y_BL2, X_BL2}, p_float32, a_temporary, &sparse_vgg_block);
    computation *alloc = b_workspace.allocate_at(conv2_init, ffout);

    k1_range0.after_low_level(conv1_init_zero, 1);
    k1_range1.after_low_level(k1_range0, 2);
    filter_k1.after_low_level(k1_range1, 3);
    convolve1.after_low_level(filter_k1, 4);
    conv1_add_bias.after_low_level(convolve1, 3);

    maxpool_init.after_low_level(conv1_add_bias, 1);
    k2_range0.after_low_level(maxpool_init, 0);
    k2_range1.after_low_level(k2_range0, 4);
    alloc->after_low_level(k2_range1, 4);
    conv2_init.after_low_level(*alloc, 4);
    ...
    else if (WEIGHTS_DENSITY < 0.05)
      convolve1.vectorize(xx, 8);
    ...
    buffer b_result("b_result", {BATCH_SIZE, F_Out, N/2, N/2}, p_float32, a_output, &sparse_vgg_block);
    ...
    conv1_init_zero.set_access("{conv1_init_zero[b,fout,output_y,output_x]->b_conv1[b, fout, output_y, output_x]}");

    conv1_add_bias.set_access("{conv1_add_bias[b,fout,y_b,x_b,yy,xx]->b_conv1[b, fout, y_b *"+Y_BL1s+" + yy + 1, x_b *"+X_BL1s+" + xx + 1]}");

    convolve1.set_access("{convolve1[b,fout,y_b,x_b,k1,yy,xx]->b_conv1[b, fout, y_b *"+Y_BL1s+" + yy + 1, x_b *"+X_BL1s+" + xx + 1]}");


    // Second convolution computations mapping

    maxpool_init.set_access("{maxpool_init[b,fout,output_y,output_x]->b_result[b,fout,output_y,output_x]}");

    conv2_init.set_access("{conv2_init[b,fout_b,y_b,x_b,ffout,yy,xx]->b_workspace[yy,xx]}");
    ...
    convolve2.set_access("{convolve2[b,fout_b,y_b,x_b,ffout,k2,yy,xx]->b_workspace[yy,xx]}");
    ...
    sparse_vgg_block.gen_halide_obj("generated_sparse_vgg_block.o");

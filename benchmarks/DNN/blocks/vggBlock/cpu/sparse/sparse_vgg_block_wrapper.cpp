#include "generated_sparse_vgg_block.o.h"
...
int main(int, char **)
{
  ...
  Halide::Buffer<float> b_result(N/2, N/2, FOut, BATCH_SIZE);
  ...
  if (CHECK_CORRECTNESS){
    ...
    std::cout << "\n\t\tPercentage of correctness " << 100*(((double)nb_correct)/(BATCH_SIZE * FOut * (N/2) * (N/2))) << "%" << std::endl << std::endl;
  }

...
for (int n = 0; n < BATCH_SIZE; ++n)
	...
	vgg_block(parameters.raw_buffer(), input.raw_buffer(), filter.raw_buffer(), bias.raw_buffer(), conv.raw_buffer(), filter2.raw_buffer(), bias2.raw_buffer(), conv2_tiramisu.raw_buffer(),output.raw_buffer());
  ...
  for (int n = 0; n < BATCH_SIZE; ++n)
	for (int z = 0; z < FOut; ++z) 
		for (int y = 0; y < N-2*K; ++y)	       
			...
			for (int x = 0; x < N-2*K; ++x)	resultfile <<output(x, y, z, n);

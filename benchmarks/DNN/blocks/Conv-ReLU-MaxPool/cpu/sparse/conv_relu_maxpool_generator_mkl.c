    ...
    for (int fin = 0; fin < fin_size; fin++)
    {
      for (int ky = 0; ky < KK; ky++)
      {
        for (int kx = 0; kx < KK; kx++)
        {
          ...
          {
            matrix[fout][fin][ky][kx] = ((float)(rand()%256 - 128)) / 127.f;
            nnzAssigned++;
          }
          else{
            matrix[fout][fin][ky][kx] = 0;
          }
        }
      }
    }
...
int main()
{
    ...
    float conv_filter_param[FOut][FIn][K][K];
    float conv_bias_param[FOut];

    size_t conv_filter_size[] = {K, K, FIn, FOut};
    size_t conv_filter_strides[] = {1, K, K*K, K*K*FIn};
    ...
    if (SHOW_OUTPUT){
      for (int n = 0; n < BATCH_SIZE; ++n)
        for (int fout = 0; fout < FOut; ++fout){
          for (int y = 0; y < N/2; ++y){
            for (int x = 0; x < N/2; ++x)
              printf("%.17g\t", output_buf[x + y*N/2 + fout*N/2*N/2 + n*N/2*N/2*FOut]);
              printf("\n");
          }
          printf("\n\n");
        }

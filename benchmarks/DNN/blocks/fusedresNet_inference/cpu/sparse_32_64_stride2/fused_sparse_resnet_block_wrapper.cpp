#include "generated_fused_sparse_resnet_block_32_64_stride2.o.h"
...
void initRandomWeights(float* fin_values, int* filter_idx, int* filter_finptr, const int n, const int KK, const int fin_size, const int fout_size, const int NN, const int stride, const int stride2_input_transform, int seed)
{
    int nnzAssigned = 0;
        ...
        for (int ky = 0; ky < KK; ky++)
        {
          for (int kx = 0; kx < KK; kx++)
          {
            for (int ffin = 0; ffin<FIN_BL; ffin++){
              ...
              {
                if (stride == 2 && stride2_input_transform)
...
int generateCSRWeights(float **filter_values, float density, int **filter_idx, int** filter_finptr, int KK, int fin_size, int fout_size, int NN, int stride, int stride2_input_transform, int seed)
{
  ...
  *filter_finptr = (int *) malloc((fout_size + 1) * sizeof(int));
  initRandomWeights(*filter_values, *filter_idx, *filter_finptr, nNonzero, KK, fin_size, fout_size, NN, stride, stride2_input_transform, seed);
  return nNonzero;
}
...
int main(int, char **)
{
  ...
  if (IMPORT_CSR_FROM_FILE)
    ...
    used_K = K;
    n = N;
    FNNZ = generateCSRWeights(&filter_values, WEIGHTS_DENSITY, &filter_idx, &filter_finptr, K, FIn, FOut, N, STRIDE, STRIDE2_INPUT_TRANSFORMATION, 2);
  }
  if (IMPORT_CSR_FROM_FILE)
  ...
  if (IMPORT_CSR_FROM_FILE)
    ...
    used_K = K;
    n = N / STRIDE;
    FNNZ2 = generateCSRWeights(&filter_values2, WEIGHTS_DENSITY2, &filter_idx2, &filter_finptr2, K, FOut, FOut, N / STRIDE, 1, STRIDE2_INPUT_TRANSFORMATION, 5);
  }
  if (IMPORT_CSR_FROM_FILE)
  ...
  srand(3);
  // First convolution
  if (STRIDE == 2 && STRIDE2_INPUT_TRANSFORMATION)
  ...
  {
    start = rtclock();
		fused_sparse_resnet_block_32_64_stride2(
      b_SIZES.raw_buffer(),
      b_input.raw_buffer(),

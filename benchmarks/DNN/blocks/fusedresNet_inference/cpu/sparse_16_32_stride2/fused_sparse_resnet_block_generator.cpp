...
int main(int argc, char **argv)
{
  ...
  function fused_sparse_resnet_block("fused_sparse_resnet_block_16_32_stride2");
  ...
  std::string WIDTH_OUTs = std::to_string((N/STRIDE + 2)/2);
  ...
  constant IND_RANGE2("IND_RANGE2",(N/STRIDE + 2) * (N/STRIDE + 2) * FOut, p_int32, true, NULL, 0, &fused_sparse_resnet_block);
  ...
  constant height("height", N / STRIDE + 2, p_int32, true, NULL, 0, &fused_sparse_resnet_block);
  constant width("width", N / STRIDE + 2, p_int32, true, NULL, 0, &fused_sparse_resnet_block);
  ...
  constant output_height("output_height", N / STRIDE, p_int32, true, NULL, 0, &fused_sparse_resnet_block);
  constant output_width("output_width", N / STRIDE, p_int32, true, NULL, 0, &fused_sparse_resnet_block);

  constant filter_finptr_size("filter_finptr_size", expr(FOut + 1), p_int32, true, NULL, 0, &fused_sparse_resnet_block);

  // First Convolution
  #if PAD_OUTPUT
  computation result_init_zero_for_padding("[BATCHSIZE,F_Out,height,width]->{result_init_zero_for_padding[b,fout,y,x]: 0<=b<BATCHSIZE and 0<=fout<F_Out and 0<=y<height and 0<=x<width}", cast(p_float32, 0), true, p_float32, &fused_sparse_resnet_block);
  #endif
  ...
  #if STRIDE2_INPUT_TRANSFORMATION
    convolve.set_expression(convolve(b, fout_b, y_b, x_b, ffout, k, yy, xx) + c_input(b, (y_b * Y_BL + yy) * (N + 2) * STRIDE + (x_b * X_BL + xx)) * c_filter_values(k));
  #else
    convolve.set_expression(convolve(b, fout_b, y_b, x_b, ffout, k, yy, xx) + c_input(b, (y_b * Y_BL + yy) * (N + 2) * STRIDE + (x_b * X_BL + xx) * STRIDE) * c_filter_values(k));
  #endif
  ...
  convolve2.set_expression(convolve2(b, fout_b, y_b, x_b, ffout, k, yy, xx) + c_input2_view(b, (y_b * Y_BL2 + yy) * (N/STRIDE + 2) + x_b * X_BL2 + xx) * c_filter_values2(k));
  ...
  #if PAD_OUTPUT
    result_init_zero_for_padding.after_low_level(conv_output_init_zero, computation::root_dimension);
    k_range0.after_low_level(result_init_zero_for_padding, computation::root_dimension);
  #else
  ...
  init_conv.split(xx, vec_len1, xx1, xx2);
  init_conv.tag_vector_level(xx2, vec_len1);

  convolve.split(xx, vec_len1, xx1, xx2);
  convolve.tag_vector_level(xx2, vec_len1);

  #if !NO_BATCH
    convolve2.parallelize(b);
  #else
    convolve2.parallelize(fout_b);
  #endif

  init_conv2.split(xx, vec_len2, xx1, xx2);
  init_conv2.tag_vector_level(xx2, vec_len2);

  convolve2.split(xx, vec_len2, xx1, xx2);
  convolve2.tag_vector_level(xx2, vec_len2);
  // ---------------------------------------------------------------------------------
  // Layer III
  // ---------------------------------------------------------------------------------
  // Input
  ...
  buffer b_input("b_input", {BATCHSIZE, expr(F_In)*expr(N+2)*expr(N+2)}, p_float32, a_input, &fused_sparse_resnet_block);
  ...
  buffer b_conv1_output("b_conv1_output", {BATCH_SIZE, FOut, N/STRIDE + 2, N/STRIDE + 2}, p_float32, a_input, &fused_sparse_resnet_block);
  ...
  // Output
    ...
    buffer b_result("b_result", {BATCHSIZE, F_Out, N/STRIDE + 2, N/STRIDE + 2}, p_float32, a_output, &fused_sparse_resnet_block);
  #else
    buffer b_result("b_result", {BATCHSIZE, F_Out, N/STRIDE, N/STRIDE}, p_float32, a_output, &fused_sparse_resnet_block);
  #endif
  ...
  init_conv2.set_access("{init_conv2[b,fout_b,y_b,x_b,ffout,yy,xx]->b_workspace2[yy,xx]}");
    ...
    result_init_zero_for_padding.set_access("{result_init_zero_for_padding[b,fout,y,x]->b_result[b,fout,y,x]}");
      ...
      store_conv_bn2.set_access("[width]->{store_conv_bn2[b,fout_b,y_b,x_b,ffout,yy,xx]->b_result[b,fout_b * "+FOUT_BL2s+"+ffout,y_b * "+Y_BL2s+"+ yy + 1,((xx + 1)%2) * "+WIDTH_OUTs+" + (x_b * "+X_BL2s+" + xx + 1)/2]}");
    #else
      store_conv_bn2.set_access("{store_conv_bn2[b,fout_b,y_b,x_b,ffout,yy,xx]->b_result[b,fout_b * "+FOUT_BL2s+"+ffout,y_b * "+Y_BL2s+"+ yy + 1,x_b * "+X_BL2s+" + xx + 1]}");
    #endif
  #else
  ...
  fused_sparse_resnet_block.gen_halide_obj("generated_fused_sparse_resnet_block_16_32_stride2.o");

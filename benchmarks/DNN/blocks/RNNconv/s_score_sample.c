...
static dnnError_t simple_net(int want_groups_conv) {
    ...
    for (int i=0; i<outputSize[0]*outputSize[1]*outputSize[2]*outputSize[3]; i++) user_o[i] = 9;
    ...
    if (cv_conv1_to_user_output) CHECK_ERR( dnnConversionExecute_F32(cv_conv1_to_user_output, resConv1[dnnResourceDst], user_o), err );

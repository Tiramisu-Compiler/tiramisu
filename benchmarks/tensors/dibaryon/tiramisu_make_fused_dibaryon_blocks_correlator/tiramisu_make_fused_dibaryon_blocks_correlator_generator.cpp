...
void generate_function(std::string name)
{
   ...
   input C_r("C_r",      {r, mc, n, t}, p_float64);
   input C_i("C_i",      {r, mc, n, t}, p_float64);
   ...
   input snk_psi_re("snk_psi_re", {xc1, xc2, n}, p_float64);
   input snk_psi_im("snk_psi_im", {xc1, xc2, n}, p_float64);
    ...
    computation B2_Blocal_r1_r_init("B2_Blocal_r1_r_init", {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime}, expr((double) 0));
    computation B2_Blocal_r1_i_init("B2_Blocal_r1_i_init", {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime}, expr((double) 0));

    computation B2_Bsingle_r1_r_init("B2_Bsingle_r1_r_init", {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x}, expr((double) 0));
    computation B2_Bsingle_r1_i_init("B2_Bsingle_r1_i_init", {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x}, expr((double) 0));
    computation B2_Bdouble_r1_r_init("B2_Bdouble_r1_r_init", {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x}, expr((double) 0));
    computation B2_Bdouble_r1_i_init("B2_Bdouble_r1_i_init", {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x}, expr((double) 0));
    ...
    for (int jc = 0; jc < Nc; jc++) {
      for (int js = 0; js < Ns; js++) {
        ...
        complex_computation q_computation(
            ...
            {t, iCprime, iSprime, kCprime, kSprime, x2, y},
            B2_Q_exprs_r1[jc][js]);

        complex_expr q = q_computation(t, iCprime, iSprime, kCprime, kSprime, x2, y);

        // define local block
        complex_expr blocal_update_def = 
          B2_Blocal_r1_init(t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime) +
          q * B2_prop(1, t, jCprime, jSprime, jc, js, x2, y) * src_psi_B2;
        complex_computation blocal_update(
            // name
            str_fmt("B2_blocal_update_r1_%d_%d", jc, js),
            // iterator
            {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, y},
            // definition
            blocal_update_def);
        ...
        // define single block
        complex_expr bsingle_update_def =
          B2_Bsingle_r1_init(t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x) +
          q * B2_prop(1, t, jCprime, jSprime, jc, js, x, y) * src_psi_B2;
        complex_computation bsingle_update(
            str_fmt("B2_bsingle_update_r1_%d_%d", jc, js),
            // iterator
            {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x, y},
            // predicate
            (iCprime != kCprime || iSprime != kSprime),
            // definition
            bsingle_update_def);
    ...
    for (int ic = 0; ic < Nc; ic++) {
      for (int is = 0; is < Ns; is++) {
        ...
        complex_computation o_computation(
            ...
            // iterators
            {t, jCprime, jSprime, kCprime, kSprime, x2, y},
            B2_O_exprs_r1[ic][is]);

        complex_expr o = o_computation(t, jCprime, jSprime, kCprime, kSprime, x2, y);

        complex_expr bdouble_update_def =
          B2_Bdouble_r1_init(t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x) +
          o * B2_prop(0, t, iCprime, iSprime, ic, is, x, y) * src_psi_B2;
        complex_computation bdouble_update(
            // name
            str_fmt("B2_bdouble_o_update_r1_%d_%d", ic, is),
            // iterator
            {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x, y},
            // definition
            bdouble_update_def);
    ...
    for (int kc = 0; kc < Nc; kc++) {
      for (int ks = 0; ks < Ns; ks++) {
        ...
        complex_computation p_computation(
            ...
            // definition
            B2_P_exprs_r1[kc][ks]);
        complex_expr p = p_computation(t, jCprime, jSprime, kCprime, kSprime, x2, y);
    ...
    computation B1_Blocal_r2_r_init("B1_Blocal_r2_r_init", {t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime}, expr((double) 0));
    computation B1_Blocal_r2_i_init("B1_Blocal_r2_i_init", {t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime}, expr((double) 0));

    computation B1_Bsingle_r2_r_init("B1_Bsingle_r2_r_init", {t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime, x2}, expr((double) 0));
    computation B1_Bsingle_r2_i_init("B1_Bsingle_r2_i_init", {t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime, x2}, expr((double) 0));
    computation B1_Bdouble_r2_r_init("B1_Bdouble_r2_r_init", {t, jCprime, jSprime, kCprime, kSprime, x, m, iCprime, iSprime, x2}, expr((double) 0));
    computation B1_Bdouble_r2_i_init("B1_Bdouble_r2_i_init", {t, jCprime, jSprime, kCprime, kSprime, x, m, iCprime, iSprime, x2}, expr((double) 0));
    ...
    for (int jc = 0; jc < Nc; jc++) {
      for (int js = 0; js < Ns; js++) {
        ...
        // define local block
        complex_expr blocal_update_def = 
          B1_Blocal_r2_init(t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime) +
          q * B1_prop(1, t, jCprime, jSprime, jc, js, x, y) * src_psi_B1;
        complex_computation blocal_update(
            // name
        ...
        // define single block
        complex_expr bsingle_update_def =
          B1_Bsingle_r2_init(t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime, x2) +
          q * B1_prop(1, t, jCprime, jSprime, jc, js, x2, y) * src_psi_B1;
        complex_computation bsingle_update(
    ...
    for (int ic = 0; ic < Nc; ic++) {
      for (int is = 0; is < Ns; is++) {
        ...
        complex_expr bdouble_update_def =
          B1_Bdouble_r2_init(t, jCprime, jSprime, kCprime, kSprime, x, m, iCprime, iSprime, x2) +
          o * B1_prop(0, t, iCprime, iSprime, ic, is, x2, y) * src_psi_B1;
        complex_computation bdouble_update(
            // name
    ...
    computation B2_Blocal_r2_r_init("B2_Blocal_r2_r_init", {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime}, expr((double) 0));
    computation B2_Blocal_r2_i_init("B2_Blocal_r2_i_init", {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime}, expr((double) 0));

    computation B2_Bsingle_r2_r_init("B2_Bsingle_r2_r_init", {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x}, expr((double) 0));
    computation B2_Bsingle_r2_i_init("B2_Bsingle_r2_i_init", {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x}, expr((double) 0));
    computation B2_Bdouble_r2_r_init("B2_Bdouble_r2_r_init", {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x}, expr((double) 0));
    computation B2_Bdouble_r2_i_init("B2_Bdouble_r2_i_init", {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x}, expr((double) 0));
    ...
    for (int jc = 0; jc < Nc; jc++) {
      for (int js = 0; js < Ns; js++) {
        ...
        // define local block
        complex_expr blocal_update_def = 
          B2_Blocal_r2_init(t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime) +
          q * B2_prop(1, t, jCprime, jSprime, jc, js, x2, y) * src_psi_B2;
        complex_computation blocal_update(
            // name
        ...
        // define single block
        complex_expr bsingle_update_def =
          B2_Bsingle_r2_init(t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x) +
          q * B2_prop(1, t, jCprime, jSprime, jc, js, x, y) * src_psi_B2;
        complex_computation bsingle_update(
    ...
    for (int ic = 0; ic < Nc; ic++) {
      for (int is = 0; is < Ns; is++) {
        ...
        complex_expr bdouble_update_def =
          B2_Bdouble_r2_init(t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x) +
          o * B2_prop(0, t, iCprime, iSprime, ic, is, x, y) * src_psi_B2;
        complex_computation bdouble_update(
            // name
    ...
    computation C_init_r("C_init_r", {t, mc, r, n}, expr((double) 0));
    computation C_init_i("C_init_i", {t, mc, r, n}, expr((double) 0));
    
    int b=0;
    /* r1, b = 0 */
    complex_computation new_term_0_r1_b1("new_term_0_r1_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Blocal_r1_init(t, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0)));
    //new_term_0_r1_b1.add_predicate(snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 0);
    //new_term_0_r1_b1.add_predicate(snk_blocks(r, 0) == 1);
    new_term_0_r1_b1.add_predicate((snk_blocks(r, 0) == 1) && (snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 0));
    //new_term_0_r1_b1.add_predicate(snk_blocks(r, 0) == 1);
    complex_computation new_term_1_r1_b1("new_term_1_r1_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Blocal_r1_init(t, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1)));
    new_term_1_r1_b1.add_predicate((snk_blocks(r, 1) == 1) && (snk_b(nperm, 0, b) == 1 && snk_b(nperm, 1, b) == 1 && snk_b(nperm, 2, b) == 1));
    complex_computation new_term_2_r1_b1("new_term_2_r1_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Bsingle_r1_init(t, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0), xc2));
    new_term_2_r1_b1.add_predicate((snk_blocks(r, 0) == 1) && (snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 1 && snk_b(nperm, 2, b) == 0));
    complex_computation new_term_3_r1_b1("new_term_3_r1_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Bsingle_r1_init(t, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1), xc1));
    new_term_3_r1_b1.add_predicate((snk_blocks(r, 1) == 1) && (snk_b(nperm, 0, b) == 1 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 1));
    complex_computation new_term_4_r1_b1("new_term_4_r1_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Bdouble_r1_init(t, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), xc2));
    ...
    complex_computation new_term_5_r1_b1("new_term_5_r1_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Bdouble_r1_init(t, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), xc1));
    ...
    /* r2, b = 0 */
    complex_computation new_term_0_r2_b1("new_term_0_r2_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Blocal_r2_init(t, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0)));
    new_term_0_r2_b1.add_predicate((snk_blocks(r, 0) == 2) && (snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 0));
    complex_computation new_term_1_r2_b1("new_term_1_r2_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Blocal_r2_init(t, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1)));
    new_term_1_r2_b1.add_predicate((snk_blocks(r, 1) == 2) && (snk_b(nperm, 0, b) == 1 && snk_b(nperm, 1, b) == 1 && snk_b(nperm, 2, b) == 1));
    complex_computation new_term_2_r2_b1("new_term_2_r2_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Bsingle_r2_init(t, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0), xc2));
    new_term_2_r2_b1.add_predicate((snk_blocks(r, 0) == 2) && (snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 1 && snk_b(nperm, 2, b) == 0));
    complex_computation new_term_3_r2_b1("new_term_3_r2_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Bsingle_r2_init(t, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1), xc1));
    new_term_3_r2_b1.add_predicate((snk_blocks(r, 1) == 2) && (snk_b(nperm, 0, b) == 1 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 1));
    complex_computation new_term_4_r2_b1("new_term_4_r2_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Bdouble_r2_init(t, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), xc2));
    ...
    complex_computation new_term_5_r2_b1("new_term_5_r2_b1", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Bdouble_r2_init(t, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), xc1));
    ...
    b=1;
    /* r1, b = 1 */
    complex_computation new_term_0_r1_b2("new_term_0_r1_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Blocal_r1_init(t, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0)));
    new_term_0_r1_b2.add_predicate((snk_blocks(r, 0) == 1) && (snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 0));
    complex_computation new_term_1_r1_b2("new_term_1_r1_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Blocal_r1_init(t, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1)));
    new_term_1_r1_b2.add_predicate((snk_blocks(r, 1) == 1) && (snk_b(nperm, 0, b) == 1 && snk_b(nperm, 1, b) == 1 && snk_b(nperm, 2, b) == 1));
    complex_computation new_term_2_r1_b2("new_term_2_r1_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Bsingle_r1_init(t, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0), xc2));
    new_term_2_r1_b2.add_predicate((snk_blocks(r, 0) == 1) && (snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 1 && snk_b(nperm, 2, b) == 0));
    complex_computation new_term_3_r1_b2("new_term_3_r1_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Bsingle_r1_init(t, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1), xc1));
    new_term_3_r1_b2.add_predicate((snk_blocks(r, 1) == 1) && (snk_b(nperm, 0, b) == 1 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 1));
    complex_computation new_term_4_r1_b2("new_term_4_r1_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Bdouble_r1_init(t, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), xc2));
    ...
    complex_computation new_term_5_r1_b2("new_term_5_r1_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Bdouble_r1_init(t, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), xc1));
    ...
    /* r2, b = 1 */
    complex_computation new_term_0_r2_b2("new_term_0_r2_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Blocal_r2_init(t, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0)));
    new_term_0_r2_b2.add_predicate((snk_blocks(r, 0) == 2) && (snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 0));
    complex_computation new_term_1_r2_b2("new_term_1_r2_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Blocal_r2_init(t, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1)));
    new_term_1_r2_b2.add_predicate((snk_blocks(r, 1) == 2) && (snk_b(nperm, 0, b) == 1 && snk_b(nperm, 1, b) == 1 && snk_b(nperm, 2, b) == 1));
    complex_computation new_term_2_r2_b2("new_term_2_r2_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Bsingle_r2_init(t, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0), xc2));
    new_term_2_r2_b2.add_predicate((snk_blocks(r, 0) == 2) && (snk_b(nperm, 0, b) == 0 && snk_b(nperm, 1, b) == 1 && snk_b(nperm, 2, b) == 0));
    complex_computation new_term_3_r2_b2("new_term_3_r2_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Bsingle_r2_init(t, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1), xc1));
    new_term_3_r2_b2.add_predicate((snk_blocks(r, 1) == 2) && (snk_b(nperm, 0, b) == 1 && snk_b(nperm, 1, b) == 0 && snk_b(nperm, 2, b) == 1));
    complex_computation new_term_4_r2_b2("new_term_4_r2_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B1_Bdouble_r2_init(t, snk_color_weights(r, nperm, wnum, 1, 0), snk_spin_weights(r, nperm, wnum, 1, 0), snk_color_weights(r, nperm, wnum, 2, 0), snk_spin_weights(r, nperm, wnum, 2, 0), xc1, mc, snk_color_weights(r, nperm, wnum, 0, 0), snk_spin_weights(r, nperm, wnum, 0, 0), xc2));
    ...
    complex_computation new_term_5_r2_b2("new_term_5_r2_b2", {t, xc1, xc2, mc, r, nperm, wnum}, B2_Bdouble_r2_init(t, snk_color_weights(r, nperm, wnum, 1, 1), snk_spin_weights(r, nperm, wnum, 1, 1), snk_color_weights(r, nperm, wnum, 2, 1), snk_spin_weights(r, nperm, wnum, 2, 1), xc2, mc, snk_color_weights(r, nperm, wnum, 0, 1), snk_spin_weights(r, nperm, wnum, 0, 1), xc1));
    ...
    complex_expr term_res_b1 = new_term_0_r1_b1(t, xc1, xc2, mc, r, nperm, wnum);
    complex_expr term_res_b2 = new_term_0_r1_b2(t, xc1, xc2, mc, r, nperm, wnum);

    complex_expr snk_psi(snk_psi_re(xc1, xc2, n), snk_psi_im(xc1, xc2, n));

    complex_expr term_res = prefactor * term_res_b1 * term_res_b2 * snk_psi;

    //computation C_update_r("C_update_r", {t, xc1, xc2, mc, r, nperm, wnum, n}, C_r(t, mc, r, n) + term_res.get_real());
    //computation C_update_i("C_update_i", {t, xc1, xc2, mc, r, nperm, wnum, n}, C_i(t, mc, r, n) + term_res.get_imag());
    computation C_update_r("C_update_r", {t, xc1, xc2, mc, r, nperm, wnum, n}, C_init_r(t, mc, r, n) + term_res.get_real());
    computation C_update_i("C_update_i", {t, xc1, xc2, mc, r, nperm, wnum, n}, C_init_i(t, mc, r, n) + term_res.get_imag());
    ...
    handle = &(handle
        ->then(B2_Blocal_r2_r_init, jSprime)
        .then(B2_Blocal_r2_i_init, jSprime)
        .then(B2_Bsingle_r2_r_init, jSprime)
        .then(B2_Bsingle_r2_i_init, x2)
        .then(B2_Bdouble_r2_r_init, x2)
        .then(B2_Bdouble_r2_i_init, x2)
	);
    ...
    handle->then( *(new_term_0_r1_b1.get_real()), t)
...
//      edge.bd_r->tag_vector_level(x2, Vsnk); // Disabled due to a an error
    ...
    for (auto computations: B2_Blocal_r1_updates) {
      ...
      computation *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B2_Blocal_r1_r, {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime});
      imag->store_in(&buf_B2_Blocal_r1_i, {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime});
    }
    for (auto computations: B2_Bsingle_r1_updates) {
      computation *real;
      computation *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B2_Bsingle_r1_r, {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x});
      imag->store_in(&buf_B2_Bsingle_r1_i, {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x});
    }
    for (auto computations : B2_Bdouble_r1_o_updates) {
      computation *real, *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B2_Bdouble_r1_r, {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x});
      imag->store_in(&buf_B2_Bdouble_r1_i, {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x});
    }
    ...
    buffer buf_B1_Blocal_r2_r("buf_B1_Blocal_r2_r",   {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns}, p_float64, a_temporary);
    buffer buf_B1_Blocal_r2_i("buf_B1_Blocal_r2_i",   {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns}, p_float64, a_temporary);
    buffer buf_B1_Bsingle_r2_r("buf_B1_Bsingle_r2_r", {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns, Vsnk}, p_float64, a_temporary);
    buffer buf_B1_Bsingle_r2_i("buf_B1_Bsingle_r2_i", {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns, Vsnk}, p_float64, a_temporary);
    buffer buf_B1_Bdouble_r2_r("buf_B1_Bdouble_r2_r", {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns, Vsnk}, p_float64, a_temporary);
    buffer buf_B1_Bdouble_r2_i("buf_B1_Bdouble_r2_i", {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns, Vsnk}, p_float64, a_temporary);

    buffer buf_B2_Blocal_r2_r("buf_B2_Blocal_r2_r",   {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns}, p_float64, a_temporary);
    buffer buf_B2_Blocal_r2_i("buf_B2_Blocal_r2_i",   {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns}, p_float64, a_temporary);
    buffer buf_B2_Bsingle_r2_r("buf_B2_Bsingle_r2_r", {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns, Vsnk}, p_float64, a_temporary);
    buffer buf_B2_Bsingle_r2_i("buf_B2_Bsingle_r2_i", {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns, Vsnk}, p_float64, a_temporary);
    buffer buf_B2_Bdouble_r2_r("buf_B2_Bdouble_r2_r", {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns, Vsnk}, p_float64, a_temporary);
    buffer buf_B2_Bdouble_r2_i("buf_B2_Bdouble_r2_i", {Lt, Nc, Ns, Nc, Ns, Vsnk, Nsrc, Nc, Ns, Vsnk}, p_float64, a_temporary);

    B1_Blocal_r2_r_init.store_in(&buf_B1_Blocal_r2_r);
    B1_Blocal_r2_i_init.store_in(&buf_B1_Blocal_r2_i);

    B1_Bsingle_r2_r_init.store_in(&buf_B1_Bsingle_r2_r);
    B1_Bsingle_r2_i_init.store_in(&buf_B1_Bsingle_r2_i);

    B1_Bdouble_r2_r_init.store_in(&buf_B1_Bdouble_r2_r);
    B1_Bdouble_r2_i_init.store_in(&buf_B1_Bdouble_r2_i);
    ...
    allocate_complex_buffers(B1_q_r2_r_buf, B1_q_r2_i_buf, { Lt, Vsrc }, "buf_B1_q_r2");
    allocate_complex_buffers(B1_o_r2_r_buf, B1_o_r2_i_buf, { Lt, Vsrc }, "buf_B1_o_r2");
    allocate_complex_buffers(B1_p_r2_r_buf, B1_p_r2_i_buf, { Lt, Vsrc }, "buf_B1_p_r2");

    B2_Blocal_r2_r_init.store_in(&buf_B2_Blocal_r2_r);
    B2_Blocal_r2_i_init.store_in(&buf_B2_Blocal_r2_i);

    B2_Bsingle_r2_r_init.store_in(&buf_B2_Bsingle_r2_r);
    B2_Bsingle_r2_i_init.store_in(&buf_B2_Bsingle_r2_i);

    B2_Bdouble_r2_r_init.store_in(&buf_B2_Bdouble_r2_r);
    B2_Bdouble_r2_i_init.store_in(&buf_B2_Bdouble_r2_i);
    ...
    allocate_complex_buffers(B2_q_r2_r_buf, B2_q_r2_i_buf, { Lt, Vsrc }, "buf_B2_q_r2");
    allocate_complex_buffers(B2_o_r2_r_buf, B2_o_r2_i_buf, { Lt, Vsrc }, "buf_B2_o_r2");
    allocate_complex_buffers(B2_p_r2_r_buf, B2_p_r2_i_buf, { Lt, Vsrc }, "buf_B2_p_r2");

    for (auto edge : B1_q2userEdges_r2) {
      edge.q_r->store_in(B1_q_r2_r_buf, {t, y});
      edge.q_i->store_in(B1_q_r2_i_buf, {t, y});
    }
    for (auto edge : B1_o2userEdges_r2) {
      edge.o_r->store_in(B1_o_r2_r_buf, {t, y});
      edge.o_i->store_in(B1_o_r2_i_buf, {t, y});
    }
    for (auto edge : B1_p2userEdges_r2) {
      edge.p_r->store_in(B1_p_r2_r_buf, {t, y});
      edge.p_i->store_in(B1_p_r2_i_buf, {t, y});
    }
    ...
    for (auto computations: B1_Blocal_r2_updates) {
      ...
      computation *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B1_Blocal_r2_r, {t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime});
      imag->store_in(&buf_B1_Blocal_r2_i, {t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime});
    }
    for (auto computations: B1_Bsingle_r2_updates) {
      computation *real;
      computation *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B1_Bsingle_r2_r, {t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime, x2});
      imag->store_in(&buf_B1_Bsingle_r2_i, {t, iCprime, iSprime, kCprime, kSprime, x, m, jCprime, jSprime, x2});
    }
    for (auto computations : B1_Bdouble_r2_o_updates) {
      computation *real, *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B1_Bdouble_r2_r, {t, jCprime, jSprime, kCprime, kSprime, x, m, iCprime, iSprime, x2});
      imag->store_in(&buf_B1_Bdouble_r2_i, {t, jCprime, jSprime, kCprime, kSprime, x, m, iCprime, iSprime, x2});
    }
    ...
    for (auto edge : B2_q2userEdges_r2) {
      edge.q_r->store_in(B2_q_r2_r_buf, {t, y});
      edge.q_i->store_in(B2_q_r2_i_buf, {t, y});
    }
    for (auto edge : B2_o2userEdges_r2) {
      edge.o_r->store_in(B2_o_r2_r_buf, {t, y});
      edge.o_i->store_in(B2_o_r2_i_buf, {t, y});
    }
    for (auto edge : B2_p2userEdges_r2) {
      edge.p_r->store_in(B2_p_r2_r_buf, {t, y});
      edge.p_i->store_in(B2_p_r2_i_buf, {t, y});
    }
    ...
    for (auto computations: B2_Blocal_r2_updates) {
      ...
      computation *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B2_Blocal_r2_r, {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime});
      imag->store_in(&buf_B2_Blocal_r2_i, {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime});
    }
    for (auto computations: B2_Bsingle_r2_updates) {
      computation *real;
      computation *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B2_Bsingle_r2_r, {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x});
      imag->store_in(&buf_B2_Bsingle_r2_i, {t, iCprime, iSprime, kCprime, kSprime, x2, m, jCprime, jSprime, x});
    }
    for (auto computations : B2_Bdouble_r2_o_updates) {
      computation *real, *imag;
      std::tie(real, imag) = computations;
      real->store_in(&buf_B2_Bdouble_r2_r, {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x});
      imag->store_in(&buf_B2_Bdouble_r2_i, {t, jCprime, jSprime, kCprime, kSprime, x2, m, iCprime, iSprime, x});
    }
    ...
    snk_psi_re.store_in(&buf_snk_psi_re, {xc1, xc2, n});
    snk_psi_im.store_in(&buf_snk_psi_im, {xc1, xc2, n});

    C_r.store_in(&buf_C_r);
    C_i.store_in(&buf_C_i);
    C_init_r.store_in(&buf_C_r, {t, mc, r, n});
    C_init_i.store_in(&buf_C_i, {t, mc, r, n});
    C_update_r.store_in(&buf_C_r, {t, mc, r, n});
    C_update_i.store_in(&buf_C_i, {t, mc, r, n});

   ...
   if (Nsrc > 0) {
      if (Nsnk > 0) {
       ...
       double* B1_Blocal_r1_re = malloc(Nt * Nc * Ns * Nc * Ns * Vsnk * Nc * Ns * Nsrc * sizeof (double));
  	    ...
  	    make_local_block(B2_Blocal_r2_re, B2_Blocal_r2_im, B2_prop_re, B2_prop_im, src_color_weights_r2, src_spin_weights_r2, src_weights_r2, src_psi_B2_re, src_psi_B2_im, Nc,Ns,Vsrc,Vsnk,Nt,Nw,Nq,Nsrc);
  		 ...
  		 printf("made local/single/double blocks in %4.9f sec \n", (end_time - start_time));
	  ...
	  printf("made dibaryon correlators in %4.9f sec \n", (end_time - start_time));
         ...
         for (m=0; m<Nsrc; m++) {
            for (n=0; n<Nsnk; n++) {
               for (t=0; t<Nt; t++) {
                  C_re[index_4d(0,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_0_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk*Vsnk));
                  C_im[index_4d(0,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_0_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk*Vsnk));
                  C_re[index_4d(1,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_r1_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk*Vsnk));
                  C_im[index_4d(1,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_r1_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk*Vsnk));
                  C_re[index_4d(2,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_r2_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk*Vsnk));
                  C_im[index_4d(2,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_r2_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk*Vsnk));
                  C_re[index_4d(3,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_r3_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk*Vsnk));
                  C_im[index_4d(3,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_r3_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk*Vsnk));
               }
            }
         }
	 }
	 ...
	 {
     	 double start_time = rtclock();
       tiramisu_wrapper_make_fused_blocks_dibaryon_correlator(C_re,
           C_im,
           B1_prop_re, 
           B1_prop_im, 
           B2_prop_re, 
	 ...
	 if (USE_REFERENCE && USE_TIRAMISU && CHECK_CORRECTNESS_MAKE_CORRELATOR)
	 {
	       ...
	       for (t=0; t<Nt; t++) {
		    for (m=0; m<Nsrc; m++) {
			 for (n=0; n<Nsnk; n++) {
			    printf("Tiramisu = %lf, Ref = %lf\n", C_re[index_4d(0,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)], BB_0_re[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)));
			    if (fabs(BB_0_re[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)) - C_re[index_4d(0,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)]) >= ERROR_THRESH)
				error_msg("Results for BB_0_re do not match");
			    if (fabs(BB_0_im[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)) - C_im[index_4d(0,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)]) >= ERROR_THRESH)
				error_msg("Results for BB_0_im do not match");
			    if (fabs(BB_r1_re[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)) - C_re[index_4d(1,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)]) >= ERROR_THRESH)
				error_msg("Results for BB_r1_re do not match");
			    if (fabs(BB_r1_im[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)) - C_im[index_4d(1,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)]) >= ERROR_THRESH)
				error_msg("Results for BB_r1_im do not match");
			    if (fabs(BB_r2_re[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)) - C_re[index_4d(2,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)]) >= ERROR_THRESH)
				error_msg("Results for BB_r2_re do not match");
			    if (fabs(BB_r2_im[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)) - C_im[index_4d(2,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)]) >= ERROR_THRESH)
				error_msg("Results for BB_r2_im do not match");
			    if (fabs(BB_r3_re[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)) - C_re[index_4d(3,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)]) >= ERROR_THRESH)
				error_msg("Results for BB_r3_re do not match");
			    if (fabs(BB_r3_im[index_3d(m,n,t,Nsnk,Nt)]/(1.0*(Vsrc*Vsrc)*(Vsnk*Vsnk)) - C_im[index_4d(3,m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)]) >= ERROR_THRESH)
      ...
      if (NsnkHex > 0) {
         ...
         for (m=0; m<Nsrc; m++) {
            for (n=0; n<NsnkHex; n++) {
               for (t=0; t<Nt; t++) {
                  C_re[index_4d(0,m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_0_re[index_3d(m,n,t ,NsnkHex,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                  C_im[index_4d(0,m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_0_im[index_3d(m,n,t ,NsnkHex,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                  C_re[index_4d(1,m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r1_re[index_3d(m,n,t ,NsnkHex,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                  C_im[index_4d(1,m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r1_im[index_3d(m,n,t ,NsnkHex,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                  C_re[index_4d(2,m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r2_re[index_3d(m,n,t ,NsnkHex,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                  C_im[index_4d(2,m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r2_im[index_3d(m,n,t ,NsnkHex,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                  C_re[index_4d(3,m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r3_re[index_3d(m,n,t ,NsnkHex,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                  C_im[index_4d(3,m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r3_im[index_3d(m,n,t ,NsnkHex,Nt)]/((Vsrc*Vsrc)*(Vsnk));
               }
            }
         } 
         if (snk_entangled != 0 && Nsrc == Nsnk && NsrcHex == NsnkHex) {
            for (m=0; m<NsrcHex; m++) {
               for (n=0; n<Nsnk; n++) {
                  for (t=0; t<Nt; t++) {
                     C_re[index_4d(0,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_0_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                     C_im[index_4d(0,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = -BB_H_0_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                     C_re[index_4d(1,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r1_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                     C_im[index_4d(1,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = -BB_H_r1_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                     C_re[index_4d(2,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r2_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                     C_im[index_4d(2,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = -BB_H_r2_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                     C_re[index_4d(3,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = BB_H_r3_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                     C_im[index_4d(3,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = -BB_H_r3_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc*Vsrc)*(Vsnk));
                  }
               }
            }
         } 
   ...
   if (NsrcHex > 0) {
      if (NsnkHex > 0) {
         ...
         for (m=0; m<NsrcHex; m++) {
            for (n=0; n<NsnkHex; n++) {
               for (t=0; t<Nt; t++) {
                  C_re[index_4d(0,Nsrc+m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_0_re[index_3d(m,n,t ,Nsnk,Nt)]/((Vsrc)*(Vsnk));
                  C_im[index_4d(0,Nsrc+m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_0_im[index_3d(m,n,t ,Nsnk,Nt)]/((Vsrc)*(Vsnk));
                  C_re[index_4d(1,Nsrc+m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_r1_re[index_3d(m,n,t ,Nsnk,Nt)]/((Vsrc)*(Vsnk));
                  C_im[index_4d(1,Nsrc+m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_r1_im[index_3d(m,n,t ,Nsnk,Nt)]/((Vsrc)*(Vsnk));
                  C_re[index_4d(2,Nsrc+m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_r2_re[index_3d(m,n,t ,Nsnk,Nt)]/((Vsrc)*(Vsnk));
                  C_im[index_4d(2,Nsrc+m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_r2_im[index_3d(m,n,t ,Nsnk,Nt)]/((Vsrc)*(Vsnk));
                  C_re[index_4d(3,Nsrc+m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_r3_re[index_3d(m,n,t ,Nsnk,Nt)]/((Vsrc)*(Vsnk));
                  C_im[index_4d(3,Nsrc+m,Nsnk+n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_r3_im[index_3d(m,n,t ,Nsnk,Nt)]/((Vsrc)*(Vsnk));
               }
            }
         }
         double end_time = rtclock();
      ...
      if (Nsnk > 0) {
         if (snk_entangled == 0) {
            ...
            for (m=0; m<NsrcHex; m++) {
               for (n=0; n<Nsnk; n++) {
                  for (t=0; t<Nt; t++) {
                     C_re[index_4d(0,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_BB_0_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc)*(Vsnk*Vsnk));
                     C_im[index_4d(0,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_BB_0_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc)*(Vsnk*Vsnk));
                     C_re[index_4d(1,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_BB_r1_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc)*(Vsnk*Vsnk));
                     C_im[index_4d(1,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_BB_r1_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc)*(Vsnk*Vsnk));
                     C_re[index_4d(2,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_BB_r2_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc)*(Vsnk*Vsnk));
                     C_im[index_4d(2,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_BB_r2_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc)*(Vsnk*Vsnk));
                     C_re[index_4d(3,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_BB_r3_re[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc)*(Vsnk*Vsnk));
                     C_im[index_4d(3,Nsrc+m,n,t ,Nsrc+NsrcHex,Nsnk+NsnkHex,Nt)] = H_BB_r3_im[index_3d(m,n,t,Nsnk,Nt)]/((Vsrc)*(Vsnk*Vsnk));
                  }
               }
            }
	         double end_time = rtclock();
   ...
   for (wnum=0; wnum<Nw; wnum++) {
      ...
      for (m=0; m<Nsrc; m++) {
         for (n=0; n<Nsnk; n++) {
            for (t=0; t<Nt; t++) {
               for (x=0; x<Vsnk; x++) {
                  C_re[index_4d(0,m,n,t ,Nsrc,Nsnk,Nt)] += (src_weights_r1[wnum] * (snk_psi_re[index_2d(x,n ,Nsnk)] * Blocal_r1_re[Blocal_index(t,iC,iS,kC,kS,x,jC,jS,m ,Nc,Ns,Vsnk,Nsrc)] - snk_psi_im[index_2d(x,n ,Nsnk)] * Blocal_r1_im[Blocal_index(t,iC,iS,kC,kS,x,jC,jS,m ,Nc,Ns,Vsnk,Nsrc)]))/((Vsrc)*(Vsnk));
                  C_im[index_4d(0,m,n,t ,Nsrc,Nsnk,Nt)] += (src_weights_r1[wnum] * (snk_psi_re[index_2d(x,n ,Nsnk)] * Blocal_r1_im[Blocal_index(t,iC,iS,kC,kS,x,jC,jS,m ,Nc,Ns,Vsnk,Nsrc)] - snk_psi_im[index_2d(x,n ,Nsnk)] * Blocal_r1_re[Blocal_index(t,iC,iS,kC,kS,x,jC,jS,m ,Nc,Ns,Vsnk,Nsrc)]))/((Vsrc)*(Vsnk));
               }
            }
         }
      }
   ...
   for (wnum=0; wnum<Nw; wnum++) {
      ...
      for (m=0; m<Nsrc; m++) {
         for (n=0; n<Nsnk; n++) {
            for (t=0; t<Nt; t++) {
               for (x=0; x<Vsnk; x++) {
                  C_re[index_4d(1,m,n,t ,Nsrc,Nsnk,Nt)] += (src_weights_r2[wnum] * (snk_psi_re[index_2d(x,n ,Nsnk)] * Blocal_r2_re[Blocal_index(t,iC,iS,kC,kS,x,jC,jS,m ,Nc,Ns,Vsnk,Nsrc)] - snk_psi_im[index_2d(x,n ,Nsnk)] * Blocal_r2_im[Blocal_index(t,iC,iS,kC,kS,x,jC,jS,m ,Nc,Ns,Vsnk,Nsrc)]))/((Vsrc)*(Vsnk));
                  C_im[index_4d(1,m,n,t ,Nsrc,Nsnk,Nt)] += (src_weights_r2[wnum] * (snk_psi_re[index_2d(x,n ,Nsnk)] * Blocal_r2_im[Blocal_index(t,iC,iS,kC,kS,x,jC,jS,m ,Nc,Ns,Vsnk,Nsrc)] - snk_psi_im[index_2d(x,n ,Nsnk)] * Blocal_r2_re[Blocal_index(t,iC,iS,kC,kS,x,jC,jS,m ,Nc,Ns,Vsnk,Nsrc)]))/((Vsrc)*(Vsnk));
               }
            }
         }
      }

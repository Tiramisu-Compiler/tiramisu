...
void generate_function(std::string name)
{
    ...
    input Blocal_r("Blocal_r", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x}, p_float64);
    input Blocal_i("Blocal_i", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x}, p_float64);
    input   prop_r("prop_r",   {t, tri, jCprime, jSprime, iCprime, iSprime, x, y}, p_float64);
    input   prop_i("prop_i",   {t, tri, jCprime, jSprime, iCprime, iSprime, x, y}, p_float64);
    ...
    computation Blocal_r_init("Blocal_r_init", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x}, expr((double) 0));
    computation Blocal_i_init("Blocal_i_init", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x}, expr((double) 0));

    std::pair<expr, expr> prop_0(prop_r(t, 0, color_weights(wnum, 0), spin_weights(wnum, 0), iCprime, iSprime, x, y), prop_i(t, 0, color_weights(wnum, 0), spin_weights(wnum, 0), iCprime, iSprime, x, y));
    std::pair<expr, expr> prop_2(prop_r(t, 2, color_weights(wnum, 2), spin_weights(wnum, 2), kCprime, kSprime, x, y), prop_i(t, 2, color_weights(wnum, 2), spin_weights(wnum, 2), kCprime, kSprime, x, y));
    std::pair<expr, expr> prop_0p(prop_r(t, 0, color_weights(wnum, 0), spin_weights(wnum, 0), kCprime, kSprime, x, y), prop_i(t, 0, color_weights(wnum, 0), spin_weights(wnum, 0), kCprime, kSprime, x, y));
    std::pair<expr, expr> prop_2p(prop_r(t, 2, color_weights(wnum, 2), spin_weights(wnum, 2), iCprime, iSprime, x, y), prop_i(t, 2, color_weights(wnum, 2), spin_weights(wnum, 2), iCprime, iSprime, x, y));
    ...
    expr prop_r_1 = prop_r(t, 1, color_weights(wnum, 1), spin_weights(wnum, 1), jCprime, jSprime, x, y);
    expr prop_i_1 = prop_i(t, 1, color_weights(wnum, 1), spin_weights(wnum, 1), jCprime, jSprime, x, y);
    ...
    computation Blocal_r_update("Blocal_r_update", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, y, wnum}, p_float64);
    Blocal_r_update.set_expression(Blocal_r_init(t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x) + weights(wnum) * mul_r(psi, p1));

    computation Blocal_i_update("Blocal_i_update", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, y, wnum}, p_float64);
    Blocal_i_update.set_expression(Blocal_i_init(t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x) + weights(wnum) * mul_i(psi, p1));

    computation Q_r_init("Q_r_init", {t, n, kCprime, kSprime, jCprime, jSprime, iCprime, iSprime, x, y}, expr((double) 0));
    computation Q_i_init("Q_i_init", {t, n, kCprime, kSprime, jCprime, jSprime, iCprime, iSprime, x, y}, expr((double) 0));

    computation Bsingle_r_init("Bsingle_r_init", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2}, expr((double) 0));
    computation Bsingle_i_init("Bsingle_i_init", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2}, expr((double) 0));

    computation Q_r_update("Q_r_update", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, y, wnum},
			Q_r_init(t, n, kCprime, kSprime, color_weights(wnum, 1), spin_weights(wnum, 1), iCprime, iSprime, x, y) + weights(wnum) * mul_r(psi, m1));
    Q_r_update.add_predicate((jCprime == color_weights(wnum, 1)) && (jSprime == spin_weights(wnum, 1)));

    computation Q_i_update("Q_i_update", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, y, wnum},
			Q_i_init(t, n, kCprime, kSprime, color_weights(wnum, 1), spin_weights(wnum, 1), iCprime, iSprime, x, y) + weights(wnum) * mul_i(psi, m1));
    Q_i_update.add_predicate((jCprime == color_weights(wnum, 1)) && (jSprime == spin_weights(wnum, 1)));

    std::pair<expr, expr> Q_update(Q_r_update(t, n, lCprime, lSprime, kCprime, kSprime, iCprime, iSprime, x, y, wnum), Q_i_update(t, n, lCprime, lSprime, kCprime, kSprime, iCprime, iSprime, x, y, wnum));
    std::pair<expr, expr> prop_1p(prop_r(t, 1, lCprime, lSprime, jCprime, jSprime, x2, y), prop_i(t, 1, lCprime, lSprime, jCprime, jSprime, x2, y));

    computation Bsingle_r_update("Bsingle_r_update", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, lCprime, lSprime, x, x2, y},
	    Bsingle_r_init(t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2) + mul_r(Q_update, prop_1p));

    computation Bsingle_i_update("Bsingle_i_update", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, lCprime, lSprime, x, x2, y},
	    Bsingle_i_init(t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2) + mul_i(Q_update, prop_1p));

    computation Bdouble_r_init("Bdouble_r_init", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2}, expr((double) 0));
    computation Bdouble_i_init("Bdouble_i_init", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2}, expr((double) 0));
    ...
    std::pair<expr, expr> prop_0pp(prop_r(t, 0, lCprime, lSprime, iCprime, iSprime, x2, y), prop_i(t, 0, lCprime, lSprime, iCprime, iSprime, x2, y));

    computation Bdouble_r_update0("Bdouble_r_update0", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, lCprime, lSprime, x, x2, y},
	    Bdouble_r_init(t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2) + mul_r(prop_0pp, O_update));

    computation Bdouble_i_update0("Bdouble_i_update0", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, lCprime, lSprime, x, x2, y},
	    Bdouble_i_init(t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2) + mul_i(prop_0pp, O_update));

    std::pair<expr, expr> prop_2pp(prop_r(t, 2, lCprime, lSprime, iCprime, iSprime, x2, y), prop_i(t, 2, lCprime, lSprime, iCprime, iSprime, x2, y));
    computation Bdouble_r_update1("Bdouble_r_update1", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, lCprime, lSprime, x, x2, y},
	    Bdouble_r_init(t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2) - mul_r(P_update, prop_2pp));

    computation Bdouble_i_update1("Bdouble_i_update1", {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, lCprime, lSprime, x, x2, y},
	    Bdouble_i_init(t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2) - mul_i(P_update, prop_2pp));
    ...
    Blocal_r_update.store_in(&buf_Blocal_r, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x});
    Blocal_i_update.store_in(&buf_Blocal_i, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x});
    ...
    Q_r_update.store_in(&buf_Q_r, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, y});
    Q_i_update.store_in(&buf_Q_i, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, y});
    ...
    Bsingle_r_update.store_in(&buf_Bsingle_r, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2});
    Bsingle_i_update.store_in(&buf_Bsingle_i, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2});
    Bdouble_r_update0.store_in(&buf_Bdouble_r, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2});
    Bdouble_i_update0.store_in(&buf_Bdouble_i, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2});
    Bdouble_r_update1.store_in(&buf_Bdouble_r, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2});
    Bdouble_i_update1.store_in(&buf_Bdouble_i, {t, n, jCprime, jSprime, kCprime, kSprime, iCprime, iSprime, x, x2});

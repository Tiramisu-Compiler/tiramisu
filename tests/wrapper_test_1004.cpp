...
void init_zero(obj_mat *m, int32_t dim0, int32_t dim1) {
  m->dim0 = dim0;
  m->dim1 = dim1;
  memset(m->data, 0, dim0 * dim1 * sizeof(uint64_t));
}

void init_encode(obj_mat *m, int32_t dim0, int32_t dim1) {
  m->dim0 = dim0;
  m->dim1 = dim1;
...
// NOTE: Computations with zero variables are translated to iteration domains
// with a single dimension that is fixed at 0.
void obj_init_a(halide_buffer_t *buf, int32_t zero, int32_t dim0, int32_t dim1) {
  obj_mat *m = (obj_mat *)buf->host;
  init_encode(m, dim0, dim1);
}

void obj_init_b(halide_buffer_t *buf, int32_t zero, int32_t dim0, int32_t dim1) {
  obj_mat *m = (obj_mat *)buf->host;
  init_encode(m, dim0, dim1);
}

void obj_init_c(halide_buffer_t *buf, int32_t zero, int32_t dim0, int32_t dim1) {
  obj_mat *m = (obj_mat *)buf->host;
  init_zero(m, dim0, dim1);
}

void obj_matmul(halide_buffer_t *buf_c, int32_t c_j, int32_t c_i, int32_t c_k,
...
int main(int argc, char **argv) {
  ...
  mm_obj(buf_a.raw_buffer(), buf_b.raw_buffer(), buf_c.raw_buffer());

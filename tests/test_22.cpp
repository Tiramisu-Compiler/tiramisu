...
/**
 * Test computing dependence graph and bound inference.
 */
...
void generate_function(std::string name, int size, int val0)
{
    ...
    tiramisu::computation S0("[N,M]->{S0[i,j]: 0<=i<M and 0<=j<M}", tiramisu::expr((uint8_t) val0),                 true, p_uint8, &function0);
    tiramisu::computation S1("[N,M]->{S1[i,j]: 0<=i<M and 0<=j<M}", S0(i,j),                                        true, p_uint8, &function0);
    tiramisu::computation S2("[N,M]->{S2[i,j]: 0<=i<M and 0<=j<M}", S0(i,j) + S0(i,j) + S1(0,1),            true, p_uint8, &function0);
    tiramisu::computation S3("[N,M]->{S3[i,j]: 0<=i<M and 0<=j<M}", S2(i,j) + S2(i,j) + S1(i,j) + S0(i,j),  true, p_uint8, &function0);

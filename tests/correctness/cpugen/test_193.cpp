...
void generate_function(std::string name, int size, int val0)
{
    ...
    auto locality_vector = std::get<2>(auto_skewing_result);
    //third vector of locality

    assert(locality_vector.size() > 0);

    auto first_sol = locality_vector[0];

    result.skew(i,j,first_sol.first,first_sol.second,i1,j1);
    result.loop_reversal(j1,j2);

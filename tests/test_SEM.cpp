...
void generate_function(std::string name)
{
    ...
    computation reduce_D_init_along_dim_i("[cN]->{reduce_D_init_along_dim_i[j]: 0<=j<cN}", // reduction down columns
					  expr(0.0f), true, p_float32, &function0);
    computation reduce_D_init_along_dim_j("[cN]->{reduce_D_init_along_dim_j[i]: 0<=i<cN}", // reduction across rows
					  expr(0.0f), true, p_float32, &function0);
    computation reduce_D_along_dim_i("[cN]->{reduce_D_along_dim_i[i,j]: 0<=i<cN and 0<=j<cN}", // reduction down columns
				     reduce_D_init_along_dim_i(j) + matrix_D(i,j), true, p_float32, &function0);
    computation reduce_D_along_dim_j("[cN]->{reduce_D_along_dim_j[i,j]: 0<=i<cN and 0<=j<cN}", // reduction across rows
				     reduce_D_init_along_dim_j(i) + matrix_D(i,j), true, p_float32, &function0);
    ...
    computation reduce_u_along_dim_i("[cN]->{reduce_u_along_dim_i[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
				     reduce_u_init_along_dim_i(j,k) + tensor_u(i,j,k), true, p_float32, &function0);
    computation reduce_u_along_dim_j("[cN]->{reduce_u_along_dim_j[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
				     reduce_u_init_along_dim_j(i,k) + tensor_u(i,j,k), true, p_float32, &function0);
    computation reduce_u_along_dim_k("[cN]->{reduce_u_along_dim_k[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
				     reduce_u_init_along_dim_k(i,j) + tensor_u(i,j,k), true, p_float32, &function0);
    computation tensor_w_0("[cN]->{tensor_w_0[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
			   reduce_D_along_dim_j(k,0) * reduce_u_along_dim_k(i,j,0), true, p_float32, &function0);
    computation tensor_w_1("[cN]->{tensor_w_1[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
			   reduce_D_along_dim_j(j,0) * reduce_u_along_dim_j(i,0,k), true, p_float32, &function0);
    computation tensor_w_2("[cN]->{tensor_w_2[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
			   reduce_D_along_dim_j(i,0) * reduce_u_along_dim_i(0,j,k), true, p_float32, &function0);    
    ...
    computation tensor_z_reduce_along_dim_i("[cN]->{tensor_z_reduce_along_dim_i[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
					    tensor_z_init_reduce_along_dim_i(j,k) + tensor_z(2,0,i,j,k), true, p_float32, &function0);
    computation tensor_z_reduce_along_dim_j("[cN]->{tensor_z_reduce_along_dim_j[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
					    tensor_z_init_reduce_along_dim_j(i,k) + tensor_z(1,0,i,j,k), true, p_float32, &function0);
    computation tensor_z_reduce_along_dim_k("[cN]->{tensor_z_reduce_along_dim_k[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
					    tensor_z_init_reduce_along_dim_k(i,j) + tensor_z(0,0,i,j,k), true, p_float32, &function0);
    computation tensor_u_update_init("[cN]->{tensor_u_update_init[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
				  reduce_D_along_dim_i(0,k)*tensor_z_reduce_along_dim_k(i,j,0), true, p_float32, &function0);
    computation tensor_u_update_0("[cN]->{tensor_u_update_0[i,j,k]: 0<=i<cN and 0<=j<cN and 0<=k<cN}",
				  ...
				  tensor_u_update_init(i,j,k), true, p_float32, &function0);
    ...
    tiramisu::buffer buff_u("buff_u", {N, N, N}, p_float32, a_output, &function0);
    tiramisu::buffer buff_D_reduce_along_dim_i("buff_D_reduce_along_dim_i", {N}, p_float32, a_output, &function0);
    tiramisu::buffer buff_D_reduce_along_dim_j("buff_D_reduce_along_dim_j", {N}, p_float32, a_output, &function0);
    tiramisu::buffer buff_u_reduce_along_dim_i("buff_u_reduce_along_dim_i", {N, N}, p_float32, a_output, &function0);
    tiramisu::buffer buff_u_reduce_along_dim_j("buff_u_reduce_along_dim_j", {N, N}, p_float32, a_output, &function0);
    tiramisu::buffer buff_u_reduce_along_dim_k("buff_u_reduce_along_dim_k", {N, N}, p_float32, a_output, &function0);
    tiramisu::buffer buff_tensor_w("buff_tensor_w", {3, N, N, N}, p_float32, a_output, &function0);
    tiramisu::buffer buff_tensor_z("buff_tensor_z", {3, N, N, N}, p_float32, a_output, &function0);
    tiramisu::buffer buff_z_0_reduce_along_dim_k("buff_z_0_reduce_along_dim_k", {N,N}, p_float32, a_temporary, &function0);
    tiramisu::buffer buff_z_1_reduce_along_dim_j("buff_z_1_reduce_along_dim_j", {N,N}, p_float32, a_temporary, &function0);
    tiramisu::buffer buff_z_2_reduce_along_dim_i("buff_z_2_reduce_along_dim_i", {N,N}, p_float32, a_temporary, &function0);
    ...
    reduce_D_init_along_dim_i.set_access("{reduce_D_init_along_dim_i[j]->buff_D_reduce_along_dim_i[j]}");
    reduce_D_init_along_dim_j.set_access("{reduce_D_init_along_dim_j[i]->buff_D_reduce_along_dim_j[i]}");
    reduce_D_along_dim_i.set_access("{reduce_D_along_dim_i[i,j]->buff_D_reduce_along_dim_i[j]}");
    reduce_D_along_dim_j.set_access("{reduce_D_along_dim_j[i,j]->buff_D_reduce_along_dim_j[i]}");
    ...
    function0.set_arguments({&buff_D, &buff_G, &buff_u, &buff_D_reduce_along_dim_i, &buff_D_reduce_along_dim_j, &buff_tensor_w, &buff_u_reduce_along_dim_i, &buff_u_reduce_along_dim_j, &buff_u_reduce_along_dim_k, &buff_tensor_z});

...
int main(int argc, char **argv) {
  ...
  Halide::Buffer<int64_t> buf_a(num_tasks_a, j_dim_size, "buf_a");
  Halide::Buffer<int64_t> buf_b(num_tasks_b, j_dim_size, "buf_b");
  ...
  Halide::Buffer<void *>  buf_wait_a_send(num_tasks_a, k_dim_size, j_dim_size, "buf_wait_a_send");
  Halide::Buffer<void *>  buf_wait_a_recv(num_tasks_c, j_dim_size, "buf_wait_a_recv");
  Halide::Buffer<void *>  buf_wait_b_send(num_tasks_b, i_dim_size, j_dim_size, "buf_wait_b_send");
  Halide::Buffer<void *>  buf_wait_b_recv(num_tasks_c, j_dim_size, "buf_wait_b_recv");
  Halide::Buffer<int64_t> buf_a_local(num_tasks_c, j_dim_size, "buf_a_local");
  Halide::Buffer<int64_t> buf_b_local(num_tasks_c, j_dim_size, "buf_b_local");
  ...
  */
  ...
  {
    for (uint64_t round = 0; round < j_dim_size; round++) {
      for (uint64_t map_c = rank, tsk_c = 0; map_c < i_dim_size * k_dim_size; map_c += num_ranks, tsk_c++) {
        uint64_t i = map_c / k_dim_size;
        uint64_t k = map_c % k_dim_size;

        {
          uint64_t bits_recv = buf_a_local(tsk_c, round);
          char name_recv = 0;
          uint32_t i_recv = 0, j_recv = 0, round_recv = 0;
          DECODE_POINT_ROUND(bits_recv, &name_recv, &i_recv, &j_recv, &round_recv);

          printf("RANK %02d: ROUND %02" PRIu64 ": CHECK: %c(%02" PRIu64 ", %02" PRIu64 ") = %c(%02" PRIu32 ", %02" PRIu32 ")" ,
                 rank, round, 'a', i, k, name_recv, i_recv, j_recv);
          if (round != round_recv) {
        ...
        {
          uint64_t bits_recv = buf_b_local(tsk_c, round);
          char name_recv = 0;
          uint32_t i_recv = 0, j_recv = 0, round_recv = 0;
          DECODE_POINT_ROUND(bits_recv, &name_recv, &i_recv, &j_recv, &round_recv);

          printf("RANK %02d: ROUND %02" PRIu64 ": CHECK: %c(%02" PRIu64 ", %02" PRIu64 ") = %c(%02" PRIu32 ", %02" PRIu32 ")" ,
                 rank, round, 'b', i, k, name_recv, i_recv, j_recv);
          if (round != round_recv) {
            printf(" (ROUND %02" PRIu32 ")", round_recv);
          }
          if (round != round_recv ||
...

  {
    ...
    for (uint64_t map_c = rank; map_c < i_dim_size * k_dim_size; map_c += num_ranks, tsk_c++) {
      ...
      int64_t sum = 0;
      for (uint64_t j = 0; j < j_dim_size; j++) {
        sum += (int64_t) encode_point_round('A', (uint32_t) i, (uint32_t) j, (uint32_t) j) * 
            (int64_t) encode_point_round('B', (uint32_t) j, (uint32_t) k, (uint32_t) j);
      }
      buf_c_ref(tsk_c) = sum;
    }
    for (; tsk_c < num_tasks_c; tsk_c++) {

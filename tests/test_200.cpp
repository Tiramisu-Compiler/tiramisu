...
void generate_function(std::string name, int size, int val0)
{
    ...
    tiramisu::computation addition({i,j}, A(i, j) + 3);
    ...
    fct->append_clustered_statements({&addition, &result});
    fct->sort_clustered_statements();

    // assert(fct->check_clustered_statements_are_innermost());

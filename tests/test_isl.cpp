...
/*	Computation computation0(ctx, Halide::Expr(3), "{S0[i,j]: 0<=i<=1000 and 0<=j<=1000}", &fct);
	...
	Halide::Buffer buf(Halide::Int(32), 10, 0, 0, 0, NULL, "buf");
	fct.buffers_list.insert(std::pair<std::string, Halide::Buffer *>(buf.name(), &buf));
	computation0.SetAccess("{S0[0,0]->buf[0]}");
...
*/
	...
	Halide::Argument buffer_arg("buf", Halide::Argument::OutputBuffer, Halide::Int(32), 1);

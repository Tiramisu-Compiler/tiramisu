...
void generate_function(std::string name, int size, int val0)
{
    ...
    tiramisu::buffer buf0("buf0", 2, {10, 10}, tiramisu::p_uint8, NULL, a_temporary, &function0);
    S0.set_access("[N,M]->{S0[i,j]->buf0[i,j]: 0<=i<N and 0<=j<N}");
    tiramisu::buffer buf1("buf1", 2, {10, 10}, tiramisu::p_uint8, NULL, a_temporary, &function0);
    S1.set_access("[N,M]->{S1[i,j]->buf1[i,j]: 0<=i<N and 0<=j<N}");
    tiramisu::buffer buf2("buf2", 2, {10, 10}, tiramisu::p_uint8, NULL, a_temporary, &function0);
    S2.set_access("[N,M]->{S2[i,j]->buf2[i,j]: 0<=i<N and 0<=j<N}");
    tiramisu::buffer buf3("buf3", 2, {10, 10}, tiramisu::p_uint8, NULL, a_output, &function0);

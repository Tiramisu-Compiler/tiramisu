...
void generate_function(std::string name, int size, int val0)
{
    ...
    auto auto_skewing_result = fct->skewing_local_solver({&B_out,&B_out_2,&A_out},i,j,2);
    ...
    B_out.skew(i,j,first_sol.first,first_sol.second,i1,j1);
    A_out.skew(i,j,first_sol.first,first_sol.second,i1,j1);
    B_out_2.skew(i,j,first_sol.first,first_sol.second,i1,j1);

    assert(fct->loop_parallelization_is_legal(j1,{&B_out,&B_out_2,&A_out}) == true);

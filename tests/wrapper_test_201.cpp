...
int main(int, char **)
{
	...
	for (int j = 1; j < SIZE1-1; j++) {
	    reference_buf0(j, i) = reference_buf0(j, i - 1) + reference_buf0(j - 1, i - 1)+  reference_buf0(j, i - 1);
        tmp_var = reference_buf0(j, i) + 2;
        reference_buf0(j, i) = tmp_var + 1;
    }

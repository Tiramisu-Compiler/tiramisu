...
int main(int argc, char **argv)
{
    ...
    //Computations
    computation A_sub("{A_sub[i,j,k]: 0<=i<128 and 0<=j<i and 0<=k<j}", A_sub(i,j,k) - A(i,k)*A(k,j), true, p_float64, global::get_implicit_function());
    //A_sub.set_expression(A_sub(i,j,k) - A(i,k)*A(k,j));
    computation A_div("{A_div[i,j]: 0<=i<128 and 0<=j<i}", A_sub(i,j,0)/A_sub(j,j,0), true, p_float64, global::get_implicit_function());
    //A_div.set_expression(A_sub(i,j,0)/A_sub(j,j,0));
    computation A_out("{A_out[i,l,m]: 0<=i<128 and i<=l<128 and 0<=m<i}", A_out(i,l,m) - A_div(i,m)*A_div(m,l), true, p_float64, global::get_implicit_function());

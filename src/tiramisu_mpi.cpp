...
{
    check_MPI_error(MPI_Ssend(data, count, type, dest, 0, MPI_COMM_WORLD));
}
...
make_Isend_nowait(uint8, unsigned char, MPI_UNSIGNED_CHAR)
make_Isend_nowait(int16, short, MPI_SHORT)
make_Isend_nowait(uint16, unsigned short, MPI_UNSIGNED_SHORT)
make_Isend_nowait(int32, int, MPI_INT)
make_Isend_nowait(uint32, unsigned int, MPI_UNSIGNED)
make_Isend_nowait(int64, long, MPI_LONG)
make_Isend_nowait(uint64, unsigned long, MPI_UNSIGNED_LONG)
make_Isend_nowait(f32, float, MPI_FLOAT)
make_Isend_nowait(f64, double, MPI_DOUBLE)

void tiramisu_MPI_Issend(int count, int dest, int tag, char *data, MPI_Datatype type, long *reqs) 
{
    ((MPI_Request**)reqs)[0] = (MPI_Request*)malloc(sizeof(MPI_Request));
    check_MPI_error(MPI_Issend(data, count, type, dest, 0, MPI_COMM_WORLD, ((MPI_Request**)reqs)[0]));
}
...
make_Issend_nowait(uint8, unsigned char, MPI_UNSIGNED_CHAR)
make_Issend_nowait(int16, short, MPI_SHORT)
make_Issend_nowait(uint16, unsigned short, MPI_UNSIGNED_SHORT)
make_Issend_nowait(int32, int, MPI_INT)
make_Issend_nowait(uint32, unsigned int, MPI_UNSIGNED)
make_Issend_nowait(int64, long, MPI_LONG)
make_Issend_nowait(uint64, unsigned long, MPI_UNSIGNED_LONG)
make_Issend_nowait(f32, float, MPI_FLOAT)
make_Issend_nowait(f64, double, MPI_DOUBLE)
